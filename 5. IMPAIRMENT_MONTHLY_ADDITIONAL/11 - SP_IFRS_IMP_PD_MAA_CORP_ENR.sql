...
BEGIN 
    ...
    V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || V_TABLEINSERT1 || ' 
        SET 
            PERCENTAGE = CASE 
                ...
                THEN NULL 
                ELSE CAST(A.CALC_AMOUNT AS FLOAT)/CAST(B.TOTAL_ACCOUNT AS FLOAT) 
            END
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_ENR_CURR AS 
    ...
    LOOP 
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_ENR_MAA_BASE AS 
            ...
            (
                ...
                FROM (
                    ...
                    LEFT JOIN TMP_ENR_CURR B 
                        ...
                        AND B.PD_RULE_ID = ' || V_SEGMENT.PD_RULE_ID || ' 
