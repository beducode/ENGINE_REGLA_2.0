...
BEGIN 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_IMA_IMP_COLL' || ' AS 
        SELECT *       
        FROM ' || V_TABLEINSERT2 || ' 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_SUM_COLL_AMOUNT' || ' AS 
        SELECT 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_REMAINING_AMOUNT' || ' AS 
        SELECT 
            LEFT(ACCOUNT_NUMBER, (CASE 
                WHEN POSITION(''.'' IN SUBSTRING(ACCOUNT_NUMBER FROM 8)) > 0 
                THEN POSITION(''.'' IN SUBSTRING(ACCOUNT_NUMBER FROM 8)) + 7 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_COLL_FACILITY' || ' AS 
        SELECT 
            LEFT(ACCOUNT_NUMBER, (CASE 
                WHEN POSITION(''.'' IN SUBSTRING(ACCOUNT_NUMBER FROM 8)) > 0 
                THEN POSITION(''.'' IN SUBSTRING(ACCOUNT_NUMBER FROM 8)) + 7 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_IMA_SUM_COLL' || ' AS 
        SELECT 
            B.COLL_ID
            ,C.SUM_COLLATERAL
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_IFRS_SBLC_PRORATE' || ' AS 
    ...
    LOOP 
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_SUM_OS_IDR' || ' AS 
            SELECT 
                COLL_ID
                ,SUM(CASE 
