CREATE OR REPLACE EDITIONABLE PROCEDURE "PSAK413"."SP_IFRS_PD_MAA_FLOWRATE_SMOOTH2" (
    P_RUNID         VARCHAR2 DEFAULT 'S_00000_0000',
    P_DOWNLOAD_DATE DATE ,
    P_SYSCODE       VARCHAR2 DEFAULT '0',
    P_PRC           VARCHAR2 DEFAULT 'P'
)
AS
    -- Date variables
    V_CURRDATE      DATE;
	...
	LOOP
		...
		LOOP
			...
			LOOP
				...
				INSERT INTO xtesopd  (pd_rule_id, flowrate,nexpd, predpd)
			   SELECT DATA_PD.PD_RULE_ID, NVL(FLOWRATE, 0),
			    	NVL(LEAD(FLOWRATE, 1, 999) OVER (ORDER BY BUCKET_FROM), 0),
					PREDICTED_PD
				 	FROM GTMP_PD_BASE
			   	WHERE BUCKET_FROM = V_LOOP2 AND PD_RULE_ID = DATA_PD.PD_RULE_ID ;
			...
			INSERT INTO xtesopd2 (pd_rule_id,bucket, cek )
			SELECT DATA_PD.PD_RULE_ID, v_loop1, v_check 
			FROM dual ; 
		...
		END LOOP ;  
		  INSERT INTO xpd
		  SELECT * FROM GTMP_PD_RESULT  WHERE pd_rule_id = DATA_PD.PD_RULE_ID ; 
		  COMMIT ; 
