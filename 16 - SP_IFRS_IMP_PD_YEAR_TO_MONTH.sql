...
BEGIN 
    ...
    AND CURR_DATE =  (CASE WHEN B.LAG_1MONTH_FLAG = 1 THEN ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE ELSE ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE END)';
    EXECUTE (V_STR_QUERY);
    ...
    LOOP
        ...
        ,NULL AS FL_DATE      
        ,CASE     
        WHEN ' || V_COUNT_MIN || ' = 1 THEN 1-POWER((1-PD_RATE),(CAST( ' || V_COUNT_MIN || ' AS FLOAT)/CAST( 12 AS FLOAT)))      
        ELSE (1-POWER((1-PD_RATE),(CAST( ' || V_COUNT_MIN || ' AS FLOAT)/CAST( 12 AS FLOAT)))) - (1-POWER((1-PD_RATE),(CAST((' || V_COUNT_MIN || '-1) AS FLOAT)/CAST( 12 AS FLOAT))))      
        END AS PD_RATE      
        ...
        WHERE CURR_DATE = (CASE WHEN B.LAG_1MONTH_FLAG = 1 THEN ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE ELSE ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE END)      
        AND A.CREATEDBY <> ''NFR''';
