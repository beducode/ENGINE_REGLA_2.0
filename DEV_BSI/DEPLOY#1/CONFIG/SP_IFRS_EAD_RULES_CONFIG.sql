CREATE OR REPLACE PROCEDURE PSAK413.SP_IFRS_EAD_RULES_CONFIG_DEV (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_EAD_RULES_CONFIG_DEV';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
	...
	MERGE INTO IFRS_EAD_RULES_CONFIG a
		...
		WHEN MATCHED THEN
			UPDATE SET   
				a.UPDATEDBY              = 'SP_IFRS_EAD_RULES_CONFIG_DEV',
		...
		WHEN NOT MATCHED THEN
			...
			'SP_IFRS_EAD_RULES_CONFIG_DEV',sysdate
			);
