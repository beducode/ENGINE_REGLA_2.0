CREATE OR REPLACE PROCEDURE PSAK413.SP_IFRS_PD_RULES_CONFIG_DEV (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_PD_RULES_CONFIG_DEV';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
    ...
    JOIN PSAK413.IFRS_BUCKET_HEADER  C ON a."bucket_code" = c.SYSCODE_GROUP_BUCKET 
    JOIN PSAK413.IFRS_DEFAULT_CRITERIA D ON A."default_criteria_code" = d.SYSCODE_DEFAULT_CRITERIA ) x 
    WHERE x.rn = 1 
    ) b
