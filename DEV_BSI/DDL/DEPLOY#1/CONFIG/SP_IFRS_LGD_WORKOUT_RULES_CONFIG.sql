CREATE OR REPLACE PROCEDURE PSAK413.SP_IFRS_LGD_WORKOUT_RULES_CONFIG_DEV (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_LGD_WORKOUT_RULES_CONFIG_DEV';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
    ...
    JOIN PSAK413.IFRS_SEGMENTATION_MAPPING b ON a."segment_code" = b.SYSCODE_SEGMENTATION  ) x 
    WHERE x.rn = 1 
    ) b
    ON (a.SYSCODE_workout = b."syscode_workout_period_config" )  
    WHEN MATCHED THEN
    UPDATE SET   
        a.UPDATED_BY              = 'SP_IFRS_LGD_WORKOUT_RULES_CONFIG_DEV',
    ...
    'SP_IFRS_LGD_WORKOUT_RULES_CONFIG_DEV',sysdate
    );
