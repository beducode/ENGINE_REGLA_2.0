...
BEGIN
    ...
    COMMIT;
		...
		V_STR_QUERY := ' INSERT INTO ' || V_TAB_OWNER || '.' || V_TABLEPDCONFIG || ' (DOWNLOAD_DATE, MASTERID, PMTDATE, SEQ, PRINCIPAL, SUM_PRINCIPAL)
			WITH CTE AS ( 
    ...
    -------- ====== RESULT ======
    V_QUERYS := 'SELECT * FROM ' || V_TAB_OWNER || '.' || V_TABLEPDCONFIG || ' WHERE EFF_DATE = DATE ''' || TO_CHAR(V_CURRDATE, 'YYYY-MM-DD') || '''';
