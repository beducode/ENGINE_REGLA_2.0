CREATE OR REPLACE PROCEDURE PSAK413.SP_IFRS_PD_TERM_STRUCTURE_DEV (
...
BEGIN
    -- set procedure name
    V_SP_NAME := 'SP_IFRS_PD_TERM_STRUCTURE_DEV';
   ...
   V_STR_QUERY := 'DECLARE
    ...
    -- -------- ====== RESULT ======
    V_QUERYS := 'SELECT * FROM ' || V_TAB_OWNER || '.' || V_IFRS_PD_TERM_STRUCTURE || ' WHERE EFF_DATE = DATE ''' || TO_CHAR(V_CURRDATE, 'YYYY-MM-DD') || '''';
