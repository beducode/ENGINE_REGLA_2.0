...
BEGIN
    ...
    V_TABLENAME := CASE WHEN P_PRC = 'S' THEN 'IFRS_MASTER_ACCOUNT' ELSE 'IFRS_MASTER_ACCOUNT_' || P_RUNID ||'' END;
    ...
    V_STR_SQL := 'DELETE PSAK413.IFRS_EXCEPTION_ACCOUNT WHERE EXCEPTION_ID = 1 AND DOWNLOAD_DATE = DATE ''' || TO_CHAR(V_CURRDATE, 'YYYY-MM-DD') || ''' ';   
    EXECUTE IMMEDIATE V_STR_SQL;
