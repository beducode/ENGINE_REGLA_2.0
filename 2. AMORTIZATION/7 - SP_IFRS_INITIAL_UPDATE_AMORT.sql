---- DROP PROCEDURE SP_IFRS_INITIAL_UPDATE_AMORT;

CREATE OR REPLACE PROCEDURE SP_IFRS_INITIAL_UPDATE_AMORT(
...
BEGIN 
    ...
    -------- ====== BODY ======
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'START', 'SP_IFRS_INITIAL_UPDATE_AMORT', '');

    ---- UPDATE FACILITY_NUMBER & PLAFOND FROM LIMIT
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'DEBUG', 'SP_IFRS_INITIAL_UPDATE_AMORT', 'UPDATE FACILITY_NUMBER & PLAFOND FROM LIMIT');
    ...
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'DEBUG', 'SP_IFRS_INITIAL_UPDATE_AMORT', 'UPDATE IMP FIELD FROM PREVDATE');
    ...
    ---- CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'DEBUG', 'SP_IFRS_INITIAL_UPDATE_AMORT', 'UPDATE LAST & NEXT PAYMENT DATE');

    ---- UPDATE AMORT TYPE & PRODUCT NAME
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'DEBUG', 'SP_IFRS_INITIAL_UPDATE_AMORT', 'UPDATE AMORT TYPE & PRODUCT NAME');
    ...
    ---- CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'DEBUG', 'SP_IFRS_INITIAL_UPDATE_AMORT', 'EXCHANGE RATE CURR & PREV');

    ---- UPDATE IFRS9 CLASS BASED ON SPPI & BUSINESS MODEL TEST
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'DEBUG', 'SP_IFRS_INITIAL_UPDATE_AMORT', 'UPDATE IFRS9 CLASS BASED ON SPPI & BUSINESS MODEL TEST');
    ...
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'DEBUG', 'SP_IFRS_INITIAL_UPDATE_AMORT', 'RELOAD MASTER CURRENCY TABLE');
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT3 || ' 
        ( 
            ...
            ,''SP_IFRS_INITIAL_UPDATE_AMORT'' AS CREATEDBY 
            ,CURRENT_TIMESTAMP AS CREATEDDATE 
        FROM IFRS_MASTER_EXCHANGE_RATE 
        WHERE DOWNLOAD_DATE = ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE ';
    EXECUTE (V_STR_QUERY);

    ----? ORIGINAL IMPLEMENTATION USING TEMPORARY TABLE FOR B AND UPDATE A LEFT JOIN B
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || V_TABLENAME || ' A 
        SET 
    ...
    ---- UPDATE GL_CONSTNAME --* ORIGINAL SQLSERVER
    ---- CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'DEBUG', 'SP_IFRS_INITIAL_UPDATE_AMORT', 'UPDATE GL_CONSTNAME');
    ---- CALL SP_IFRS_EXEC_RULE(P_RUNID, V_CURRDATE, P_PRC, 'GL');

    ---- END
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'END', 'SP_IFRS_INITIAL_UPDATE_AMORT', '');
    ---------- ====== BODY ======
