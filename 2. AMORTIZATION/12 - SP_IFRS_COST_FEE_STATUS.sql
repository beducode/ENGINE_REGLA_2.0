---- DROP PROCEDURE SP_IFRS_COST_FEE_STATUS;

CREATE OR REPLACE PROCEDURE SP_IFRS_COST_FEE_STATUS(
    IN P_RUNID VARCHAR(20) DEFAULT 'S_00000_0000',
    IN P_DOWNLOAD_DATE DATE DEFAULT NULL,
    IN P_PRC VARCHAR(1) DEFAULT 'S',
    IN P_MAT_LEVEL INT DEFAULT 0,
    IN P_UNMAT_PL INT DEFAULT 0)
LANGUAGE PLPGSQL AS $$
DECLARE
    ---- DATE
    V_PREVDATE DATE;
...
BEGIN 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_REV_PAIR AS 
        SELECT 
            A.ID 
            ,A.CF_ID 
            ,MIN(B.ID) AS PAIR_ID 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_REV_PAIR2 
        (
            ID2 BIGINT 
            ,CF_ID2 BIGINT 
            ,PAIR_ID2 BIGINT 
