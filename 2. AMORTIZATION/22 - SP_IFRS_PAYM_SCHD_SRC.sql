...
BEGIN 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_SCHD1' || ' AS 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_SCHD2' || ' AS 
    ...
    THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_LAST_PAYM_DATE' || ' AS 
            SELECT 
                DOWNLOAD_DATE 
                ,MASTERID 
                ,CASE 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ' || 'TMP_TRANS' || ' AS 
        SELECT 
            DOWNLOAD_DATE 
            ,MASTERID 
