---- DROP PROCEDURE SP_IFRS_EXEC_RULE_AMORT;

CREATE OR REPLACE PROCEDURE SP_IFRS_EXEC_RULE_AMORT(
...
BEGIN 
    ...
    -------- ====== BODY ======
    FOR V_SEGMENT IN 
        EXECUTE 'SELECT DISTINCT UPDATED_TABLE, UPDATED_COLUMN, RULE_TYPE, TABLE_NAME, A.RULE_NAME, A.PKID AS RULE_ID
    ...
    LOOP 
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || 
            CASE WHEN V_SEGMENT.UPDATED_TABLE = 'IFRS_MASTER_ACCOUNT' THEN V_TABLENAME 
            ELSE V_SEGMENT.UPDATED_TABLE || '_' || P_RUNID END || ' A
    ...
    RAISE NOTICE 'SP_IFRS_EXEC_RULE_AMORT | AFFECTED RECORD : %', V_RETURNROWS2;
    -------- ====== BODY ======

    -------- ====== LOG ======
    V_TABLEDEST = V_TABLENAME;
    V_COLUMNDEST = '-';
    V_SPNAME = 'SP_IFRS_EXEC_RULE_AMORT';
    V_OPERATION = 'UPDATE';
