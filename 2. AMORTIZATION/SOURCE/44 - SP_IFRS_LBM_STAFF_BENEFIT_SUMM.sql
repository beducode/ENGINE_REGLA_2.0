CREATE OR REPLACE PROCEDURE SP_IFRS_LBM_STAFF_BENEFIT_SUMM(IN P_RUNID CHARACTER VARYING DEFAULT 'S_00000_0000'::CHARACTER VARYING, IN P_DOWNLOAD_DATE DATE DEFAULT NULL::DATE, IN P_PRC CHARACTER VARYING DEFAULT 'S'::CHARACTER VARYING)
 LANGUAGE PLPGSQL
AS $$
DECLARE
    ---- DATE
...
BEGIN 
    ...
    IF P_PRC = 'S' THEN 
        V_TABLEINSERT1 := 'IFRS_LBM_STAFF_BENEFIT_SUMM_' || P_RUNID || '';
        V_TABLEINSERT2 := 'IFRS_LBM_ACCT_EIR_COST_FEE_ECF_' || P_RUNID || '';
    ELSE 
        V_TABLEINSERT1 := 'IFRS_LBM_STAFF_BENEFIT_SUMM';
        V_TABLEINSERT2 := 'IFRS_LBM_ACCT_EIR_COST_FEE_ECF';
    END IF;
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT1 || ' AS SELECT * FROM IFRS_LBM_STAFF_BENEFIT_SUMM WHERE 1=0 ';
        EXECUTE (V_STR_QUERY);
    END IF;
    -------- ====== PRE SIMULATION TABLE ======

    -------- ====== BODY ======
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'START', 'SP_IFRS_LBM_STAFF_BENEFIT_SUMM', '');
    ...
    ---- END
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'END', 'SP_IFRS_LBM_STAFF_BENEFIT_SUMM', '');

    RAISE NOTICE 'SP_IFRS_LBM_STAFF_BENEFIT_SUMM | AFFECTED RECORD : %', V_RETURNROWS2;
    ---------- ====== BODY ======
