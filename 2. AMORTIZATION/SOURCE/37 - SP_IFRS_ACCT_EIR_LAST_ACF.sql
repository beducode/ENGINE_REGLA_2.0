CREATE OR REPLACE PROCEDURE SP_IFRS_ACCT_EIR_LAST_ACF(IN P_RUNID CHARACTER VARYING DEFAULT 'S_00000_0000'::CHARACTER VARYING, IN P_DOWNLOAD_DATE DATE DEFAULT NULL::DATE, IN P_PRC CHARACTER VARYING DEFAULT 'S'::CHARACTER VARYING)
 LANGUAGE PLPGSQL
AS $$
DECLARE
    ---- DATE
...
BEGIN 
    ...
    -------- ====== BODY ======
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'START', 'SP_IFRS_ACCT_EIR_LAST_ACF', '');
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || 'IFRS_ACCT_EIR_ACCRU_PREV' || ' A 
        SET 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || 'IFRS_ACCT_EIR_ACF' || ' 
        (
            DOWNLOAD_DATE 
		    ,FACNO 
		    ,CIFNO 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || 'IFRS_ACCT_EIR_ACF' || ' A 
        SET 
            N_UNAMORT_FEE = 0 
    ...
    ---- END
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'END', 'SP_IFRS_ACCT_EIR_LAST_ACF', '');

    RAISE NOTICE 'SP_IFRS_ACCT_EIR_LAST_ACF | AFFECTED RECORD : %', V_RETURNROWS2;
    ---------- ====== BODY ======
