---- DROP PROCEDURE SP_IFRS_ACCT_EIR_GS_INSERT;

CREATE OR REPLACE PROCEDURE SP_IFRS_ACCT_EIR_GS_INSERT(
...
BEGIN 
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT2 || ' AS SELECT * FROM IFRS_ACCT_EIR_ECF WHERE 1=0 ';
        EXECUTE (V_STR_QUERY);
    ...
    -------- ====== BODY ======
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'START', 'SP_IFRS_ACCT_EIR_GS_INSERT', '');
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT5 || ' 
        (
            ...
            ,CASE 
                ...
                THEN B.BENEFIT
                ELSE B.COST_AMT + B.FEE_AMT - COALESCE(B.GAIN_LOSS_FEE_AMT, 0) --201801417
                    - COALESCE(B.GAIN_LOSS_COST_AMT, 0) --201801417
            ...
            ,CASE 
                ...
                THEN CASE 
                    ...
                    ELSE 0
                END
                ELSE B.COST_AMT - COALESCE(B.GAIN_LOSS_COST_AMT, 0) --201801417 C
            ...
            ,CASE 
                ...
                THEN CASE 
                    ...
                    ELSE 0
                END
                ELSE B.FEE_AMT - COALESCE(B.GAIN_LOSS_FEE_AMT, 0) --201801417 F
            ...
            ,CASE 
                ...
                THEN CASE 
                    ...
                    ELSE 0
                END
                ELSE B.COST_AMT - COALESCE(B.GAIN_LOSS_COST_AMT, 0) --201801417
            END N_COST_AMT_PREV
        FROM ' || V_TABLEINSERT7 || ' A
        JOIN ' || 'IFRS_ACCT_EIR_CF_ECF' || ' B 
        ON B.MASTERID = A.MASTERID
        JOIN ' || 'IFRS_ACCT_EIR_GS_RESULT' || ' C 
        ...
        V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT4 || ' 
            (
                ...
                ,FN_ROUND_DOUBLE(CASE 
                    ...
                    THEN CAST(A.I_DAYS AS DOUBLE PRECISION) / CAST(360 AS DOUBLE PRECISION) * C.N_EFF_INT_RATE / 100 * C.N_FAIRVALUE
                    --WHEN A.INTCALCCODE=''3'' REMARKS FOR ALIGN WITH ICC PAYMENT SCHEDULE 20160428
                    WHEN A.INTCALCCODE IN (''2'', ''3'')
                    THEN CAST(A.I_DAYS AS DOUBLE PRECISION) / CAST(365 AS DOUBLE PRECISION) * C.N_EFF_INT_RATE / 100 * C.N_FAIRVALUE
                    WHEN A.INTCALCCODE = ''4''
                    THEN C.N_EFF_INT_RATE / C.N_INT_RATE * A.N_INT_PAYMENT
                    ELSE (CAST(30 AS DOUBLE PRECISION) * A.M / CAST(360 AS DOUBLE PRECISION) * C.N_EFF_INT_RATE / 100 * C.N_FAIRVALUE)
                END, ' || V_ROUND || ')
                AS N_EFF_INT_AMT
    ...
    ---- END
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'END', 'SP_IFRS_ACCT_EIR_GS_INSERT', '');

    RAISE NOTICE 'SP_IFRS_ACCT_EIR_GS_INSERT | AFFECTED RECORD : %', V_RETURNROWS2;
    ---------- ====== BODY ======

    -------- ====== LOG ======
    V_TABLEDEST = V_TABLEINSERT5;
    V_COLUMNDEST = '-';
    V_SPNAME = 'SP_IFRS_ACCT_EIR_GS_INSERT';
    V_OPERATION = 'INSERT';
