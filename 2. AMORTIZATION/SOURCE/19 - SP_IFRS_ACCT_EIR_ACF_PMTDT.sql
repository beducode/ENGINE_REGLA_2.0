CREATE OR REPLACE PROCEDURE SP_IFRS_ACCT_EIR_ACF_PMTDT(IN P_RUNID CHARACTER VARYING DEFAULT 'S_00000_0000'::CHARACTER VARYING, IN P_DOWNLOAD_DATE DATE DEFAULT NULL::DATE, IN P_PRC CHARACTER VARYING DEFAULT 'S'::CHARACTER VARYING)
 LANGUAGE PLPGSQL
AS $$
DECLARE
    ---- DATE
...
BEGIN 
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT5 || ' 
        (
            ...
            ,CASE 
                WHEN ' || V_FUNCROUND || ' = 0 
                THEN ROUND(CAST(CAST(B.AMOUNT AS FLOAT) / CAST(B.SUM_AMT AS FLOAT) AS NUMERIC(32, 20)) * A.N_UNAMORT_FEE, ' || V_ROUND || ') 
                ELSE TRUNC(CAST(CAST(B.AMOUNT AS FLOAT) / CAST(B.SUM_AMT AS FLOAT) AS NUMERIC(32, 20)) * A.N_UNAMORT_FEE, ' || V_ROUND || ')
            END AS N_AMOUNT 
            ,B.STATUS 
            ,CURRENT_TIMESTAMP 
            ,A.ACCTNO 
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT5 || ' 
        (
            ...
            ,CASE 
                WHEN ' || V_FUNCROUND || ' = 0
                THEN ROUND(CAST(CAST(B.AMOUNT AS FLOAT) / CAST(B.SUM_AMT AS FLOAT) AS NUMERIC(32, 20)) * A.N_UNAMORT_COST, ' || V_ROUND || ')  
                ELSE TRUNC(CAST(CAST(B.AMOUNT AS FLOAT) / CAST(B.SUM_AMT AS FLOAT) AS NUMERIC(32, 20)) * A.N_UNAMORT_COST, ' || V_ROUND || ') 
            END AS N_AMOUNT 
            ,B.STATUS 
            ,CURRENT_TIMESTAMP 
            ,A.ACCTNO 
