CREATE OR REPLACE PROCEDURE SP_IFRS_LBM_REPORT_RECON(IN P_RUNID CHARACTER VARYING DEFAULT 'S_00000_0000'::CHARACTER VARYING, IN P_DOWNLOAD_DATE DATE DEFAULT NULL::DATE, IN P_PRC CHARACTER VARYING DEFAULT 'S'::CHARACTER VARYING)
 LANGUAGE PLPGSQL
AS $$
DECLARE
    ---- DATE
...
BEGIN 
    ...
    IF P_PRC = 'S' THEN 
        ...
        V_TABLEINSERT4 := 'IFRS_LBM_REPORT_RECON_' || P_RUNID || '';
    ELSE 
        V_TABLENAME := 'IFRS_MASTER_ACCOUNT';
        V_TABLEINSERT2 := 'IFRS_ACCT_JOURNAL_DATA';
        V_TABLEINSERT3 := 'IFRS_JOURNAL_PARAM';
        V_TABLEINSERT4 := 'IFRS_LBM_REPORT_RECON';
    END IF;
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT4 || ' AS SELECT * FROM IFRS_LBM_REPORT_RECON WHERE 1=0 ';
        EXECUTE (V_STR_QUERY);
    END IF;
    -------- ====== PRE SIMULATION TABLE ======

    -------- ====== BODY ======
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'START', 'SP_IFRS_LBM_REPORT_RECON', '');
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || 'TMP_LOAN_REPORT_RECON' || ' 
        (        
            ...
            ,0 AS DAILY_AMORT_GL_BENEFIT_AMT        
            ,CASE         
                WHEN ' || V_ISBOM || ' = 0        
                THEN MTD_AMORT_GL_BENEFIT_AMT        
                ELSE 0        
            END AS MTD_AMORT_GL_BENEFIT_AMT        
            ,CASE         
                WHEN ' || V_ISBOY || ' = 0        
                THEN YTD_AMORT_GL_BENEFIT_AMT        
                ELSE 0        
            END AS YTD_AMORT_GL_BENEFIT_AMT        
            ,A.AMORT_GL_FAIRVALUE_AMT        
            ,0 AS DAILY_AMORT_GL_FAIRVALUE_AMT        
            ,CASE         
                WHEN ' || V_ISBOM || ' = 0        
                THEN MTD_AMORT_GL_FAIRVALUE_AMT        
                ELSE 0        
            END AS MTD_AMORT_GL_FAIRVALUE_AMT        
            ,CASE         
                WHEN ' || V_ISBOY || ' = 0        
                THEN YTD_AMORT_GL_FAIRVALUE_AMT        
                ELSE 0        
            END AS YTD_AMORT_GL_FAIRVALUE_AMT        
            ,METHOD        
    ...
    V_STR_QUERY := V_STR_QUERY || 'MERGE INTO ' || V_TABLEINSERT4 || ' A
        ...
        ON (
            ...
            ,OUTSTANDING_BANK = B.OUTSTANDING_BANK        
            ,PLAFOND = B.PLAFOND        
            ,UNAMORT_MASTER_AMT = COALESCE(B.UNAMORT_BENEFIT, 0)  
            ,METHOD = B.AMORT_TYPE ';
    EXECUTE (V_STR_QUERY);
    ...
    V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || V_TABLEINSERT4 || ' 
        SET 
            COUNTER_CHECK_BENEFIT = ROUND((SUMMARY.INITIAL_GL_BENEFIT_AMT - SUMMARY.UNAMORT_GL_BENEFIT_AMT - SUMMARY.AMORT_GL_BENEFIT_AMT) + (A.UNAMORT_MASTER_AMT - SUMMARY.UNAMORT_GL_BENEFIT_AMT), ' || V_ROUND || ')        
            ,COUNTER_CHECK_FAIRVALUE = ROUND((SUMMARY.INITIAL_GL_FAIRVALUE_AMT - SUMMARY.UNAMORT_GL_FAIRVALUE_AMT - SUMMARY.AMORT_GL_FAIRVALUE_AMT) + (A.UNAMORT_MASTER_AMT + SUMMARY.UNAMORT_GL_FAIRVALUE_AMT), ' || V_ROUND || ')        
        FROM ' || V_TABLEINSERT4 || ' A        
        INNER JOIN (        
            SELECT 
                MASTERID        
    ...
    ---- END
    CALL SP_IFRS_LOG_AMORT(V_CURRDATE, 'END', 'SP_IFRS_LBM_REPORT_RECON', '');

    RAISE NOTICE 'SP_IFRS_LBM_REPORT_RECON | AFFECTED RECORD : %', V_RETURNROWS2;
    ---------- ====== BODY ======
