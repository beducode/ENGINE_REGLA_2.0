CREATE OR REPLACE PROCEDURE SP_IFRS_PAYM_SCHD_SRC(IN P_RUNID CHARACTER VARYING DEFAULT 'S_00000_0000'::CHARACTER VARYING, IN P_DOWNLOAD_DATE DATE DEFAULT NULL::DATE, IN P_PRC CHARACTER VARYING DEFAULT 'S'::CHARACTER VARYING)
 LANGUAGE PLPGSQL
AS $$
DECLARE
    ---- DATE
...
BEGIN 
    ...
    THEN
        ...
        V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || V_TABLEINSERT5 || ' A 
            ...
            WHERE A.MASTERID = B.MASTERID 
            AND A.COUNTER = 1 
            AND RIGHT(A.MASTERID, 3)::VARCHAR NOT IN (SELECT DISTINCT VALUE1 FROM TBLM_COMMONCODEDETAIL WHERE COMMONCODE = ''SCM010'') ';
        EXECUTE (V_STR_QUERY);
    END IF;
