...
BEGIN 
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT2 || ' 
        (    
            ...
            ,M.DATA_SOURCE    
            ,CASE 
                WHEN CAST(((''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE - A.PREV_PMT_DATE) + 1) AS DOUBLE PRECISION) / CAST(A.I_DAYS2 AS DOUBLE PRECISION) > 1
                THEN (A.N_COST_UNAMORT_AMT - A.N_COST_UNAMORT_AMT_PREV)
                ELSE ROUND(CAST(CAST(((''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE - A.PREV_PMT_DATE) + 1) AS DOUBLE PRECISION) / CAST(A.I_DAYS2 AS DOUBLE PRECISION) * (A.N_COST_UNAMORT_AMT - A.N_COST_UNAMORT_AMT_PREV) AS NUMERIC), ' || V_ROUND || ')
            END + A.N_COST_UNAMORT_AMT_PREV
            ,CASE 
                WHEN CAST(((''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE - A.PREV_PMT_DATE) + 1) AS DOUBLE PRECISION) / CAST(A.I_DAYS2 AS DOUBLE PRECISION) > 1
                THEN (A.N_FEE_UNAMORT_AMT - A.N_FEE_UNAMORT_AMT_PREV)
                ELSE ROUND(CAST(CAST(((''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE - A.PREV_PMT_DATE) + 1) AS DOUBLE PRECISION) / CAST(A.I_DAYS2 AS DOUBLE PRECISION) * (A.N_FEE_UNAMORT_AMT - A.N_FEE_UNAMORT_AMT_PREV) AS NUMERIC), ' || V_ROUND || ')
            ...
            ,CASE     
                ...
                ELSE ROUND(CAST(CAST(((''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE - A.PREV_PMT_DATE) + 1) AS DOUBLE PRECISION) / CAST(A.I_DAYS2 AS DOUBLE PRECISION) * A.NOCF_AMORT_AMT AS NUMERIC), ' || V_ROUND || ') 
            END    
            ,A.NOCF_UNAMORT_AMT    
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT4 || ' 
        (    
            ...
            ,B.TRXCODE    
            ,B.CCY    
            ,ROUND(CAST(CAST(B.AMOUNT AS DOUBLE PRECISION) / CAST(B.SUM_AMT AS DOUBLE PRECISION) AS NUMERIC(32, 20)) * A.N_UNAMORT_FEE, ' || V_ROUND || ') AS N_AMOUNT    
            ,B.STATUS    
            ,CURRENT_TIMESTAMP    
            ,A.ACCTNO    
            ,A.MASTERID    
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT4 || ' 
        (    
            ...
            ,B.TRXCODE    
            ,B.CCY    
            ,ROUND(CAST(CAST(B.AMOUNT AS DOUBLE PRECISION) / CAST(B.SUM_AMT AS DOUBLE PRECISION) AS NUMERIC(32, 20)) * A.N_UNAMORT_COST, ' || V_ROUND || ') AS N_AMOUNT    
            ,B.STATUS    
            ,CURRENT_TIMESTAMP    
            ,A.ACCTNO    
            ,A.MASTERID    
