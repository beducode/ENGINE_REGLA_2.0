CREATE OR REPLACE PROCEDURE SP_IFRS_NOMINATIVE_CARD_W (
...
BEGIN
    ...
    IF v_RUNNING_MODE = 0 THEN
        DELETE /*+ PARALLEL(8) */ IFRS.IFRS_NOMINATIVE WHERE REPORT_DATE = V_CURRDATE AND DATA_SOURCE = 'CRD';
    ELSE
        DELETE /*+ PARALLEL(8) */ IFRS.IFRS_NOMINATIVE WHERE REPORT_DATE = V_CURRDATE AND DATA_SOURCE = 'CRD'
    ...
    INSERT /*+ PARALLEL(8) */ INTO IFRS.IFRS_NOMINATIVE (MASTERID,
      ...
      SELECT /*+ PARALLEL(8) */ A.MASTERID                                                                           AS MASTERID,
             ...
             CASE WHEN A.RESERVED_VARCHAR_24 IS NULL THEN
                  A.ACCOUNT_NUMBER
                ELSE
             ...
             NVL (A.PLAFOND, 0)                                                                   AS LIMIT_AMT_CCY,
             NVL (A.PLAFOND, 0) * NVL (A.EXCHANGE_RATE, 1)                                        AS LIMIT_AMT_LCL,
             NVL (A.RESERVED_AMOUNT_2, 0)                                                         AS UNUSED_AMT_CCY,
             NVL (A.RESERVED_AMOUNT_2, 0) * NVL (A.EXCHANGE_RATE, 1)                              AS UNUSED_AMT_LCL,
