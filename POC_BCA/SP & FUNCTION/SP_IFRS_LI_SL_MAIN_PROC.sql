CREATE OR REPLACE PROCEDURE SP_IFRS_LI_SL_MAIN_PROC
AS
  V_CURRDATE DATE;
  V_PREVDATE DATE;
...
BEGIN
    ...
    *******************************************************************************/
    ...
    VALUES(  V_CURRDATE,SYSTIMESTAMP,'START','SP_IFRS_LI_SL_MAIN_PROC','');
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','CLEAN UP done');

    COMMIT;

    /******************************************************************************
    02. INSERT INTO IFRS_LI_ACF_SL_MSTR
    *******************************************************************************/
    INSERT INTO IFRS_LI_ACF_SL_MSTR
    ( EFFDATE,
      ACCOUNT_NUMBER,
      TRX_CODE,
      CCY,
    ...
    WHERE TRX_CODE IN (SELECT DISTINCT TRX_CODE
                       FROM (SELECT DISTINCT TRX_CODE FROM IFRS_LI_TRANSACTION_PARAM WHERE AMORT_TYPE = 'SL') A
                       LEFT JOIN (SELECT DISTINCT TRX_CODE AS TRX_CODE_EIR FROM IFRS_LI_TRANSACTION_PARAM  WHERE AMORT_TYPE = 'EIR') B
    ...
    *******************************************************************************/
    MERGE INTO  IFRS_LI_ACF_SL_MSTR A
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','INS 1 done');

    COMMIT;
    /******************************************************************************
    04. INSERT INTO IFRS_LI_ACF_SL_MSTR (SL)
    *******************************************************************************/
    INSERT INTO IFRS_LI_ACF_SL_MSTR
    ( EFFDATE,
      ACCOUNT_NUMBER,
    ...
    SELECT V_CURRDATE,
           ...
           ORG_CCY_AMT/EQV_LCY_AMT
    FROM IFRS_LI_TRANSACTION_DAILY A
    JOIN IFRS_LI_TRANSACTION_PARAM B
      ON  A.TRX_CODE = B.TRX_CODE
      AND (A.PRD_CODE = B.PRD_CODE OR NVL(B.PRD_CODE,'ALL')= 'ALL')
      AND AMORT_TYPE = 'SL'
    WHERE A.TRX_CODE IN (SELECT DISTINCT TRX_CODE
                         FROM (SELECT DISTINCT TRX_CODE FROM IFRS_LI_TRANSACTION_PARAM WHERE AMORT_TYPE = 'SL') A
                         JOIN (SELECT DISTINCT TRX_CODE AS TRX_CODE_EIR FROM IFRS_LI_TRANSACTION_PARAM WHERE AMORT_TYPE = 'EIR' ) B
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','INS 2 done');

    /******************************************************************************
    05. UPDATE IFRS_LI_ACF_SL_MSTR
    *******************************************************************************/
    MERGE INTO IFRS_LI_ACF_SL_MSTR A
    USING IFRS_LI_TRANSACTION_PARAM B
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','UPDATE done');
    ...
    06. CAPTURE REVERSAL
    *******************************************************************************/
    INSERT INTO IFRS_LI_ACF_SL_MSTR_REV
    ( MASTERID
     ,TRX_CODE
     ,CCY
     ,PRD_CODE
    ...
    *******************************************************************************/
    MERGE INTO IFRS_LI_ACF_SL_MSTR_REV A
    USING IFRS_LI_MASTER_ACCOUNT B
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','REVERSE done');

    DELETE FROM IFRS_LI_ACF_SL_MSTR
    WHERE EFFDATE = V_CURRDATE
    AND ((ORIGINAL_VALUE < 0 AND FLAG_CF = 'C') OR (ORIGINAL_VALUE > 0 AND FLAG_CF = 'F'))  ;
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','UPDATE 2 done');
    /*
    ...
    */
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','INS ACCT COST FEE done');
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','INS SL MSTR prevdate done');

    --reverse
    --mark the ACF_SL_MSTR first

    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_REV_LS';

    INSERT INTO TMP_LI_REV_LS
    ( ACCOUNT_NUMBER,
      TRX_CODE,
      FLAG_CF,
      AMOUNT,
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_CF_LS';

    INSERT INTO TMP_LI_CF_LS
    (
      ACCOUNT_NUMBER,
      TRX_CODE,
      FLAG_CF,
    ...
    FROM (SELECT A.ACCOUNT_NUMBER, A.TRX_CODE, A.FLAG_CF, ORIGINAL_VALUE_ORG, A.ID_SL, 1 AS CNT
          ...
          JOIN ( SELECT DISTINCT ACCOUNT_NUMBER, TRX_CODE, AMOUNT*-1 AS AMT FROM TMP_LI_REV_LS) B
    ...
    MERGE INTO TMP_LI_REV_LS A
    USING TMP_LI_CF_LS B
    ON (A.CNT = B.CNT
        AND A.ACCOUNT_NUMBER = B.ACCOUNT_NUMBER
        AND A.TRX_CODE = B.TRX_CODE
    ...
    MERGE INTO TMP_LI_CF_LS A
    USING TMP_LI_REV_LS B
    ON (A.CNT = B.CNT
            AND A.ACCOUNT_NUMBER = B.ACCOUNT_NUMBER
            AND A.TRX_CODE = B.TRX_CODE
            AND A.AMOUNT = B.AMOUNT*-1
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','REVERSE 2 done');
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','REVERSE INS ACCT COST FEE done');
    ...
    SET CLOSING_AMOUNT = -ORIGINAL_VALUE,
        ...
        AMORT_VALUE_ORG = -ORIGINAL_VALUE_ORG,
        IFRS_STATUS = 'CLS'
    WHERE MASTERID NOT IN (SELECT MASTERID FROM IFRS_LI_MASTER_ACCOUNT
                           WHERE DOWNLOAD_DATE = V_CURRDATE)
    AND IFRS_STATUS = 'ACT'
    AND EFFDATE = V_CURRDATE;
    ...
    SET CLOSING_AMOUNT = -ORIGINAL_VALUE,
        ...
        AMORT_VALUE_ORG = -ORIGINAL_VALUE_ORG,
        IFRS_STATUS = 'CLS'
    WHERE MASTERID IN (SELECT MASTERID FROM IFRS_LI_MASTER_ACCOUNT
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_IMA_LS';

    INSERT INTO TMP_LI_IMA_LS(MASTERID)
    ...
    */
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','CLOSE ACCT done');
    ...
    where a.download_date = @currdate
     ...
     group by masterid ) b on a.MASTER_ACCOUNT_ID = b.masterid
      where a.download_Date = @currdate
      */
    ...
    SET A.UNAMORT_FEE_AMT = CASE WHEN B.FLAG_CF = 'F' THEN B.UNAMORT_VALUE_ORG ELSE 0 END,
        A.UNAMORT_COST_AMT= CASE WHEN B.FLAG_CF = 'C' THEN B.UNAMORT_VALUE_ORG ELSE 0 END;


    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_SL_MAIN_PROC','UPD IMA done');
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'END','SP_IFRS_LI_SL_MAIN_PROC','');
