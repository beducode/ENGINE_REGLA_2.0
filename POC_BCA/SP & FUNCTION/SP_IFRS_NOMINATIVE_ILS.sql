CREATE OR REPLACE PROCEDURE SP_IFRS_NOMINATIVE_ILS (v_DOWNLOADDATECUR DATE DEFAULT ('1-JAN-1900'),v_DOWNLOADDATEPREV DATE DEFAULT ('1-JAN-1900'),v_RUNNING_MODE NUMBER DEFAULT 0)
    AS
           V_CURRDATE DATE;
    ...
    IF v_RUNNING_MODE = 0 THEN
        DELETE /*+ PARALLEL(8) */ IFRS.IFRS_NOMINATIVE WHERE REPORT_DATE = V_CURRDATE AND DATA_SOURCE = 'ILS';
    ELSE
        DELETE /*+ PARALLEL(8) */ IFRS.IFRS_NOMINATIVE WHERE REPORT_DATE = V_CURRDATE AND DATA_SOURCE = 'ILS'
    ...
    SELECT /*+ PARALLEL(8) */ A.MASTERID                                                                                                     AS MASTERID,
           ...
           CASE WHEN A.REVOLVING_FLAG = 1 THEN
              'Y'
           ...
           WHEN A.PRODUCT_CODE LIKE 'B%' THEN
               case
                   ...
                   when length(A.ACCOUNT_NUMBER) = 21 then
                        SUBSTR(A.ACCOUNT_NUMBER, 1, 10) || '-' ||
                        SUBSTR(A.ACCOUNT_NUMBER, 11, 3) || '-' ||
                        SUBSTR(A.ACCOUNT_NUMBER, 14, 3) || '-' ||
                        SUBSTR(A.ACCOUNT_NUMBER, 17, 5)
           ...
           CASE WHEN NVL(A.NPL_FLAG, '0') = '0' THEN
              'N'
             ELSE
              'Y'
           END                                                                                                            AS NPL_FLAG,
           ...
           CASE WHEN NVL(A.BTB_FLAG, '0') = '0' THEN
              'N'
             ELSE
              'Y'
           END                                                                                                            AS BTB_FLAG,
           ...
           (NVL(A.OUTSTANDING, 0) - NVL(C.OUTSTANDING, 0)) * NVL(A.EXCHANGE_RATE, 1)                                      AS OUTSTANDING_PRINCIPAL_DIFF,
           CASE WHEN NVL(A.FAIR_VALUE_AMOUNT, 0) > NVL(A.OUTSTANDING, 0) THEN
              NVL(A.OUTSTANDING, 0)
             ELSE
           ...
           CASE WHEN A.OUTSTANDING = 0 THEN
              0
             ELSE
           ...
           CASE WHEN NVL(A.INITIAL_UNAMORT_ORG_FEE, 0) -
                     NVL(A.UNAMORT_BENEFIT, A.UNAMORT_FEE_AMT) > 0 THEN
             NVL(A.INITIAL_UNAMORT_ORG_FEE, 0)
           ELSE
             NVL(A.UNAMORT_BENEFIT, A.UNAMORT_FEE_AMT)
           END                                                                                                            AS UNAMORT_FEE_AMT_CCY,
           CASE WHEN NVL(A.INITIAL_UNAMORT_ORG_FEE, 0) -
                     NVL(A.UNAMORT_BENEFIT, A.UNAMORT_FEE_AMT) > 0 THEN
             NVL(A.INITIAL_UNAMORT_ORG_FEE, 0)
           ELSE
             NVL(A.UNAMORT_BENEFIT, A.UNAMORT_FEE_AMT)
           END * NVL(A.EXCHANGE_RATE, 1)                                                                                  AS UNAMORT_FEE_AMT_LCL,
           ...
           (NVL(A.INITIAL_UNAMORT_TXN_COST, 0) - NVL(A.UNAMORT_COST_AMT, 0)) * NVL(A.EXCHANGE_RATE, 1)                    AS AMORT_COST_LCL,
           ...
           CASE WHEN NVL(A.POCI_FLAG, '0') = '0' THEN
              'N'
             ELSE
              'Y'
           END                                                                                                            AS POCI_FLAG,
          ...
          CASE WHEN A.RESERVED_VARCHAR_9 LIKE '%H%'
                THEN
                    1
                ELSE
                    0
                END FLAG_H,
          A.RESTRUCTURE_DATE,
          A.RESERVED_VARCHAR_16 COMPANY_NO,
          A.RESERVED_VARCHAR_4 USER_CODE_1,
          A.RESERVED_VARCHAR_6 USER_CODE_3,
          BRANCH.REGION_CODE,
          BRANCH.BRANCH_NAME,
    ...
    COMMIT;
        ...
        -----    CR-021 OVERRIDE STAGE 21 JUNE 2021 --- RAL CHANGED EOM JAN 22


    MERGE INTO IFRS.IFRS_NOMINATIVE A
    USING
    (
        SELECT DISTINCT B.CUSTOMER_NUMBER
        FROM IFRS.IFRS_NOMINATIVE B
        WHERE B.SEGMENTATION IN ('COMMERCIAL','COMMERCIAL BG','CORPORATE','CORPORATE BG') AND B.DATA_SOURCE = 'ILS' AND B.BI_COLLECTABILITY = '1'
        AND B.REPORT_DATE = v_DOWNLOADDATECUR AND B.RESERVED_FLAG_1 = 1 AND
        EXISTS (SELECT 1 FROM IFRS.TBLU_WORSTCASE_LIST C WHERE B.CUSTOMER_NUMBER = C.CUSTOMER_NUMBER)
    ) B
    ON (A.REPORT_DATE = v_DOWNLOADDATECUR AND A.CUSTOMER_NUMBER = B.CUSTOMER_NUMBER AND BI_COLLECTABILITY = '1' AND
    (SEGMENTATION IN ('COMMERCIAL','COMMERCIAL BG','CORPORATE','CORPORATE BG') AND DATA_SOURCE = 'ILS'
        AND REPORT_DATE = V_DOWNLOADDATECUR AND RESERVED_FLAG_1 = 1)
    )
    WHEN MATCHED THEN
    UPDATE SET STAGE = 1;
    ...
    SET STAGE = 1
    WHERE ((SEGMENTATION NOT IN ('COMMERCIAL','COMMERCIAL BG','CORPORATE','CORPORATE BG')) AND DATA_SOURCE = 'ILS' AND BI_COLLECTABILITY = '1' AND RESERVED_FLAG_1 = 1)
