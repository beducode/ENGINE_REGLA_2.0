CREATE OR REPLACE PROCEDURE SP_IFRS_LBM_ACCT_EIR_ECF_MERGE
AS
  V_CURRDATE DATE;
  V_PREVDATE DATE;
  V_ROUND NUMBER(10);
...
BEGIN
    ...
    SELECT MAX(PREVDATE) INTO V_PREVDATE
    FROM IFRS_PRC_DATE_AMORT;

    INSERT INTO IFRS_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES(  V_CURRDATE,SYSTIMESTAMP,'START','SP_IFRS_LBM_ACCT_EIR_ECF_MERGE','');
    COMMIT;
    ...
    INSERT /*+ PARALLEL(8) */ INTO TMP_NOCF (MASTERID,DOWNLOAD_DATE)
    SELECT /*+ PARALLEL(8) */ A.MASTERID,A.DOWNLOAD_DATE
    ...
    HAVING MIN(A.N_AMORT_AMT) < 0
        AND MAX(A.N_AMORT_AMT) > 0; -- AMORT AMT HAVE BOTH VALUES NEGTV AND POSTV
    COMMIT;

    DELETE /*+ PARALLEL(8) */ FROM IFRS_LBM_ACCT_NOCF
    WHERE DOWNLOAD_DATE = V_CURRDATE;
    COMMIT;

    INSERT /*+ PARALLEL(8) */ INTO IFRS_LBM_ACCT_NOCF (MASTERID,DOWNLOAD_DATE)
    SELECT MASTERID,DOWNLOAD_DATE FROM TMP_NOCF;
    COMMIT;

    -- UPDATE NOCF AMORT AND UNAMORT AMT FROM NOCF ECF
    MERGE /*+ PARALLEL(8) */ INTO IFRS_LBM_ACCT_EIR_ECF A
    USING (SELECT E.*
          FROM IFRS_LBM_ACCT_EIR_ECF_NOCF E
          JOIN TMP_NOCF F ON F.DOWNLOAD_DATE = E.DOWNLOAD_DATE
    ...
    MERGE /*+ PARALLEL(8) */ INTO IFRS_LBM_ACCT_EIR_ECF A
    USING (SELECT E.DOWNLOAD_DATE
              ,E.MASTERID
              ,E.PMT_DATE
              ,G.FEE_AMT
              ,G.COST_AMT
              ,COALESCE(G.BENEFIT, 0) AS BENEFIT
          FROM IFRS_LBM_ACCT_EIR_ECF E
          JOIN TMP_NOCF F ON F.DOWNLOAD_DATE = E.DOWNLOAD_DATE
              AND F.MASTERID = E.MASTERID
          JOIN
              (
                SELECT MASTERID, FEE_AMT, COST_AMT, BENEFIT
                FROM IFRS_LBM_ACCT_EIR_CF_ECF
                WHERE COALESCE(FEE_AMT,0) + COALESCE(COST_AMT,0) + COALESCE(BENEFIT,0) <> 0
              ) G ON G.MASTERID = E.MASTERID
          ) B
    ...
    WHEN MATCHED THEN
    UPDATE
    SET N_FEE_UNAMORT_AMT = N_UNAMORT_AMT * (
            B.FEE_AMT + CASE WHEN B.BENEFIT < 0 THEN B.BENEFIT ELSE 0 END
        ...
        ,N_COST_UNAMORT_AMT = N_UNAMORT_AMT * (
            B.COST_AMT + CASE WHEN B.BENEFIT > 0 THEN B.BENEFIT ELSE 0 END
            ) / (B.FEE_AMT + B.COST_AMT + B.BENEFIT)
    ...
    UPDATE /*+ PARALLEL(8) */ IFRS_LBM_ACCT_EIR_ECF
    SET N_FAIRVALUE_PREV = NOCF_OSPRN_PREV + N_UNAMORT_AMT_PREV
        ,--N_OSPRN_PREV + N_UNAMORT_AMT_PREV ,
        N_FAIRVALUE = NOCF_OSPRN + N_UNAMORT_AMT
        ,--N_OSPRN + N_UNAMORT_AMT
        N_DAILY_AMORT_COST = ROUND(CASE
    ...
    INSERT INTO IFRS_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'END','SP_IFRS_LBM_ACCT_EIR_ECF_MERGE','');
    COMMIT;
END;