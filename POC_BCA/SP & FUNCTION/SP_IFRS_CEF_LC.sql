CREATE OR REPLACE PROCEDURE SP_IFRS_CEF_LC (v_DOWNLOADDATE  DATE DEFAULT ('1-JAN-1900'))
    AS
        V_CURRDATE DATE;
        V_PREVDATE DATE;
    ...
    SELECT ADD_MONTHS(V_CURRDATE,-1) INTO V_PREVDATE FROM DUAL;
    ...
    */
    ...
    DELETE IFRS_CEF_HEADER WHERE DOWNLOAD_DATE >= V_CURRDATE;
    COMMIT;

    EXECUTE IMMEDIATE 'TRUNCATE TABLE GTMP_IFRS_MASTER_ACCOUNT';COMMIT;
    ...
    SYSDATE,
    'LOCALHOST'
    FROM (SELECT AVG(CEF_RATE)CEF_RATE FROM IFRS_CEF_PROCESS WHERE DOWNLOAD_DATE <= V_CURRDATE AND AMOUNT_OFF_BS_PREV <> 0
                );
                COMMIT;
