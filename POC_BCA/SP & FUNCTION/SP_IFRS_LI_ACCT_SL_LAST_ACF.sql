CREATE OR REPLACE PROCEDURE SP_IFRS_LI_ACCT_SL_LAST_ACF
AS
    V_CURRDATE	DATE;
    V_PREVDATE	DATE;
...
BEGIN
    ...
    *******************************************************************************/
    ...
    VALUES(V_CURRDATE,SYSTIMESTAMP,'START','SP_IFRS_LI_ACCT_SL_LAST_ACF','');
    ...
    *******************************************************************************/
    ...
    FROM IFRS_LI_ACCT_CLOSED A
    JOIN IFRS_LI_ACCT_SL_ACF B
      ON B.DOWNLOAD_DATE=V_PREVDATE
      AND B.MASTERID=A.MASTERID
    JOIN IFRS_LI_ACCT_SL_ECF C
    ...
    *******************************************************************************/
    ...
    WHERE DOWNLOAD_DATE=V_PREVDATE AND MASTERID IN (SELECT MASTERID FROM TMP_LI_T1)
    GROUP BY MASTERID;
    ...
    *******************************************************************************/
    ...
    WHERE STATUS='ACT' AND MASTERID IN (SELECT MASTERID FROM TMP_LI_T1);

    COMMIT;
    /******************************************************************************
    05.insert last acf for full amort
    *******************************************************************************/
    INSERT INTO IFRS_LI_ACCT_SL_ACF
    (DOWNLOAD_DATE
    ,FACNO
    ,CIFNO
    ,ACCTNO
    ...
    AND ID IN (SELECT ID FROM TMP_LI_P1);
    ...
    *******************************************************************************/
    ...
    /******************************************************************************
    08. UPDATE STATUS ABN IN IFRS_LI_ACCT_SL_ECF
    *******************************************************************************/
    UPDATE IFRS_LI_ACCT_SL_ECF
    ...
    COMMIT;
    /******************************************************************************
    09. UPDATE STATUS ABN IN IFRS_LI_ACCT_SL_ACCRU_PREV
    *******************************************************************************/
    UPDATE IFRS_LI_ACCT_SL_ACCRU_PREV
    SET STATUS=TO_CHAR(V_CURRDATE,'YYYYMMDD') ,CREATEDBY='SL_LAST_ACF_ABN'
    WHERE STATUS='ACT'
    AND MASTERID IN (SELECT MASTERID FROM TMP_LI_T2);
    COMMIT;
    /******************************************************************************
    ...
    *******************************************************************************/
    ...
    INSERT INTO IFRS_LI_AMORT_LOG(DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES(V_CURRDATE,SYSTIMESTAMP,'END','SP_IFRS_LI_ACCT_SL_LAST_ACF','');
    COMMIT;
