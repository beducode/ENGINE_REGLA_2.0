CREATE OR REPLACE PROCEDURE SP_IFRS_NMNATIVE_ILS_NON_REVOL(v_DOWNLOADDATECUR DATE DEFAULT ('1-JAN-1900')) AS
  V_CURRDATE DATE;
BEGIN
  ...
  for rec in (SELECT REPORT_DATE,
               ...
               WHERE DATA_SOURCE = 'ILS'
                 ...
                 and revolving_flag = 'N'
                 and account_status = 'A'
                 and upper(product_code) not like 'B%'
                 and (SEGMENT NOT IN ('EMPLOYEE LOAN',
                                      'KKB AUTOMOBIL',
                                      'KKB RODA 2',
                                      'KPR')
                      OR NVL(SEGMENT, ' ') = ' '))
