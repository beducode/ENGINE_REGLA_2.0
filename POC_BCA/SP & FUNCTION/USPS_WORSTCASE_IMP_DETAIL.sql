CREATE OR REPLACE PROCEDURE USPS_WORSTCASE_IMP_DETAIL
(
    v_downloadDate DATE,
    v_pageNumber NUMBER,
    v_pageSize NUMBER,
...
BEGIN
    ...
    ELSE
        v_Select := 'SELECT
                    ...
                    ROUND(C.EIR, 6) "EIR",
                    C.CURRENCY "Currency",
                    CASE WHEN C.DATA_SOURCE = ''KTP'' THEN ROUND(C.PRINCIPAL_AMOUNT_CCY, 6)
                    ...
                    ROUND(C.ECL_WORSTCASE_CCY, 6) "WorstCase"';
    END IF;
    ...
    --SELECT COUNT(*) INTO v_DiffDate  FROM IFRS_PRC_DATE A;
    ...
    IF(v_DiffDate <= 3) THEN
        ...
        v_Query :=  v_Select || '
                    ...
                    AND C.SPECIAL_REASON IN (''WORSTCASE'')' ||
