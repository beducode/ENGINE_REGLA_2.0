CREATE OR REPLACE PROCEDURE  USPS_TREASURYMODULEH
(
    ...
    V_QUERY :=
        'SELECT A.ACCOUNT_NUMBER, A.AMORT_TYPE,
         TO_CHAR(A.download_date, ''yyyy/MM/dd'') || A.MASTERID  || ''*'' || A.ACCOUNT_NUMBER AS DDATE_MAID,
            ...
            A.TENOR,
            A.BI_COLLECTABILITY,
             NVL(A.RESERVED_VARCHAR_12,0) as RESERVED_VARCHAR_12,
            NVL(A.RESERVED_VARCHAR_5,0) as RESERVED_VARCHAR_5,
        ...
        LEFT JOIN IFRS_MASTER_PRODUCT_PARAM P
            ON A.PRODUCT_CODE=P.PRD_CODE
        LEFT JOIN IFRS_MASTER_BRANCH B
            ON A.BRANCH_CODE = B.BRANCH_NUM
        ...
        WHERE A.DATA_SOURCE IN (SELECT VALUE1 FROM TBLM_COMMONCODEDETAIL WHERE COMMONCODE = ''B97'') '
