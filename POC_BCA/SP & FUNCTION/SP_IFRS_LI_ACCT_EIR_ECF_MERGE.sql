CREATE OR REPLACE PROCEDURE  SP_IFRS_LI_ACCT_EIR_ECF_MERGE
AS
  V_CURRDATE   DATE;
  V_PREVDATE   DATE;
  V_ROUND NUMBER(10);
...
BEGIN
    ...
    VALUES(V_CURRDATE,SYSTIMESTAMP,'START','SP_IFRS_LI_ACCT_EIR_ECF_MERGE','');
    ...
    SELECT DISTINCT MASTERID FROM IFRS_LI_ACCT_EIR_ECF_NOCF WHERE DOWNLOAD_DATE = V_CURRDATE;

    --nominate masterid to be merged (only merged when have ecfnocf) 20160518
    INSERT INTO TMP_LI_NOCF (MASTERID, DOWNLOAD_DATE)
    SELECT A.MASTERID, A.DOWNLOAD_DATE
    FROM IFRS_LI_ACCT_EIR_ECF A
    JOIN TMP_LI_T1 B ON A.MASTERID = B.MASTERID
    ...
    DELETE FROM IFRS_LI_ACCT_NOCF
    WHERE DOWNLOAD_DATE = V_CURRDATE;

    COMMIT;

    INSERT INTO IFRS_LI_ACCT_NOCF (MASTERID, DOWNLOAD_DATE)
    SELECT MASTERID, DOWNLOAD_DATE FROM TMP_LI_NOCF;
    ...
    USING (SELECT E.*
           FROM    IFRS_LI_ACCT_EIR_ECF_NOCF E
           JOIN TMP_LI_NOCF F ON F.DOWNLOAD_DATE = E.DOWNLOAD_DATE AND F.MASTERID = E.MASTERID
          ) B
   ...
   -- update fairvalue amt merged
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE ,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'END','SP_IFRS_LI_ACCT_EIR_ECF_MERGE','');
