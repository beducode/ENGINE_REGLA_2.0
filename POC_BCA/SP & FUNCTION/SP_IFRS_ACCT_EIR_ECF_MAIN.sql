...
BEGIN
    ...
    INSERT /*+ PARALLEL(12) */ INTO TMP_IFRS_ACCT_COST_FEE
    SELECT /*+ PARALLEL(12) */ * FROM IFRS_ACCT_COST_FEE
    WHERE DOWNLOAD_DATE = V_CURRDATE
      AND STATUS = 'ACT'
      AND METHOD = 'EIR'
    ...
    INSERT /*+ PARALLEL(12) */ INTO IFRS_ACCT_EIR_CF_ECF
    ( MASTERID ,
      FEE_AMT ,
      COST_AMT ,
      FEE_AMT_ACRU ,
    ...
    UPDATE /*+ PARALLEL(12) */ IFRS_ACCT_EIR_CF_ECF
    ...
    INSERT /*+ PARALLEL(12) */ INTO TMP_ACCT_EIR_COST_FEE_PREV
    (
        ID,
        DOWNLOAD_DATE,
        ECFDATE,
    ...
    THEN
        -- no accru if today is doing amort
          ...
          INSERT /*+ PARALLEL(12) */ INTO TMP_T1( MASTERID ,ACCTNO )
          SELECT DISTINCT MASTERID,ACCTNO
          FROM   /*+ PARALLEL(12) */ IFRS_ACCT_EIR_ACF
          WHERE   DOWNLOAD_DATE = V_CURRDATE
          AND DO_AMORT = 'Y';
    ...
    INSERT /*+ PARALLEL(12) */ INTO IFRS_GS_MASTERID ( MASTERID)
    SELECT /*+ PARALLEL(12) */ A.MASTERID
    FROM    TMP_T1 A;
    ...
    LOOP --loop
      ...
      INSERT /*+ PARALLEL(12) */ INTO IFRS_PAYM_CORE
      ( MASTERID ,
        ACCTNO ,
        PREV_PMT_DATE ,
        PMT_DATE ,
    ...
    UPDATE /*+ PARALLEL(12) */ IFRS_ACCT_EIR_PAYM
    ...
    -- get os
    INSERT /*+ PARALLEL(12) */ INTO TMP_B1( MASTERID ,N_OSPRN)
    SELECT /*+ PARALLEL(12) */ MASTERID ,N_OSPRN
    ...
    -- get benefit
    INSERT /*+ PARALLEL(12) */ INTO TMP_B3 ( MASTERID ,N_OSPRN ,NPV_SUM ,BENEFIT)
    SELECT /*+ PARALLEL(12) */ A.MASTERID ,
            A.N_OSPRN ,
            B.NPV_SUM ,
 ...
 --   EXECUTE IMMEDIATE  'SP_IFRS_EXEC_AND_LOG_PROCESS''SP_IFRS_ACCT_EIR_CF_ECF_GRP''';
    ...
    DELETE /*+ PARALLEL(12) */ FROM IFRS_ACCT_EIR_FAILED_GOAL_SEEK
    WHERE   DOWNLOAD_DATE = V_CURRDATE;

    COMMIT;

    DELETE /*+ PARALLEL(12) */ FROM IFRS_ACCT_EIR_GOAL_SEEK_RESULT
    WHERE   DOWNLOAD_DATE = V_CURRDATE;
    ...
    INSERT /*+ PARALLEL(12) */ INTO IFRS_ACCT_EIR_CF_ECF1
    ( MASTERID ,
      FEE_AMT ,
      COST_AMT ,
      BENEFIT ,
    ...
    DELETE /*+ PARALLEL(12) */ FROM IFRS_ACCT_EIR_GS_RESULT3
    WHERE   DOWNLOAD_DATE = V_CURRDATE;

    DELETE /*+ PARALLEL(12) */ FROM IFRS_ACCT_EIR_FAILED_GS3
    WHERE   DOWNLOAD_DATE = V_CURRDATE;
    ...
    */
    ...
    VALUES  ( V_CURRDATE ,SYSTIMESTAMP ,'DEBUG' ,'SP_IFRS_ACCT_EIR_ECF_MAIN' ,'4 GS START');
     COMMIT;
    DELETE /*+ PARALLEL(12) */ IFRS_ACCT_EIR_ECF_NOCF WHERE DOWNLOAD_DATE >= V_CURRDATE ;  -- clean up
    COMMIT;
    DELETE /*+ PARALLEL(12) */ IFRS_ACCT_EIR_GOALSEEK_RESULT4 WHERE DOWNLOAD_DATE = V_CURRDATE ;
    COMMIT;
    DELETE /*+ PARALLEL(12) */ IFRS_ACCT_EIR_FAILED_GOALSEEK4 WHERE DOWNLOAD_DATE = V_CURRDATE ;
    COMMIT;
    DELETE /*+ PARALLEL(12) */ IFRS_ACCT_EIR_GOAL_SEEK_RESULT WHERE DOWNLOAD_DATE = V_CURRDATE;
    COMMIT;
    DELETE /*+ PARALLEL(12) */ IFRS_ACCT_EIR_FAILED_GOAL_SEEK WHERE DOWNLOAD_DATE = V_CURRDATE;
    COMMIT;
    ...
    */
    ...
    SELECT /*+ PARALLEL(12) */ MASTERID
    FROM    TMP_T2
    WHERE   MASTERID NOT IN ( SELECT MASTERID FROM
                                (SELECT MASTERID FROM TMP_T1
    ...
    DELETE /*+ PARALLEL(12) */ FROM IFRS_ACCT_EIR_GAIN_LOSS
    WHERE DOWNLOAD_DATE = V_CURRDATE;

    COMMIT;

    --insert fee gain loss
    INSERT /*+ PARALLEL(12) */ INTO IFRS_ACCT_EIR_GAIN_LOSS
    ( FACNO ,
      CIFNO ,
      DOWNLOAD_DATE ,
      ECFDATE ,
    ...
    SELECT  /*+ PARALLEL(12) */ IMA.FACILITY_NUMBER ,
            IMA.CUSTOMER_NUMBER ,
            V_CURRDATE ,
            V_CURRDATE ,
            IMA.DATA_SOURCE ,
