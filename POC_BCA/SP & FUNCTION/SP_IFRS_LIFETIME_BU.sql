CREATE OR REPLACE PROCEDURE SP_IFRS_LIFETIME_BU
AS
V_CURRDATE DATE;
V_PREVDATE DATE;
...
BEGIN
    ...
    A.CUSTOMER_NAME,
    A.CUSTOMER_NUMBER
    FROM (SELECT DOWNLOAD_DATE,LIFETIME_RULE_ID,(SUBSTR(ACCOUNT_NUMBER,1,11)||PRODUCT_CODE) ACCOUNT_NUMBER, DATA_SOURCE, MAX(LOAN_DUE_DATE)LOAN_DUE_DATE, MIN(LOAN_START_DATE)LOAN_START_DATE,
       RESERVED_VARCHAR_2, REVOLVING_FLAG, MIN(DPD_START_DATE)DPD_START_DATE, MIN(RESERVED_DATE_3)RESERVED_DATE_3, MAX(RESERVED_DATE_8)RESERVED_DATE_8,
       CUSTOMER_NAME, CUSTOMER_NUMBER FROM IFRS_MASTER_ACCOUNT_MONTHLY
            WHERE DOWNLOAD_DATE = V_CURRDATE AND ACCOUNT_STATUS = 'C' AND DATA_SOURCE = 'ILS' AND REVOLVING_FLAG = 0
            GROUP BY DOWNLOAD_DATE, LIFETIME_RULE_ID, (SUBSTR(ACCOUNT_NUMBER,1,11)||PRODUCT_CODE), DATA_SOURCE, RESERVED_VARCHAR_2, REVOLVING_FLAG, CUSTOMER_NAME, CUSTOMER_NUMBER) A
    ...
    WHERE A.ACCOUNT_NUMBER NOT IN (SELECT ACCOUNT_NUMBER FROM IFRS_LIFETIME_DETAIL);
    ...
    A.CUSTOMER_NUMBER
    FROM (SELECT DOWNLOAD_DATE,LIFETIME_RULE_ID,(SUBSTR(ACCOUNT_NUMBER,1,11)||PRODUCT_CODE) ACCOUNT_NUMBER, DATA_SOURCE, MAX(LOAN_DUE_DATE)LOAN_DUE_DATE, MIN(LOAN_START_DATE)LOAN_START_DATE,
       RESERVED_VARCHAR_2, REVOLVING_FLAG, MIN(RESERVED_DATE_7)RESERVED_DATE_7, MIN(RESERVED_DATE_3)RESERVED_DATE_3, MAX(RESERVED_DATE_8)RESERVED_DATE_8,
       CUSTOMER_NAME, CUSTOMER_NUMBER FROM IFRS_MASTER_ACCOUNT_MONTHLY
            WHERE DOWNLOAD_DATE = V_CURRDATE AND REVOLVING_FLAG = 1
            AND (RESERVED_DATE_3 IS NOT NULL OR BI_COLLECTABILITY >= '3')
          AND DATA_SOURCE = 'ILS'
          GROUP BY DOWNLOAD_DATE, LIFETIME_RULE_ID, (SUBSTR(ACCOUNT_NUMBER,1,11)||PRODUCT_CODE), DATA_SOURCE, RESERVED_VARCHAR_2, REVOLVING_FLAG, CUSTOMER_NAME, CUSTOMER_NUMBER
       ) A
    --JOIN IFRS_SCENARIO_DATA B ON A.ACCOUNT_NUMBER = B.ACCOUNT_NUMBER AND A.DOWNLOAD_DATE = B.DOWNLOAD_DATE
    JOIN IFRS_LIFETIME_RULES_CONFIG C ON A.LIFETIME_RULE_ID = C.PKID
    WHERE A.ACCOUNT_NUMBER NOT IN (SELECT ACCOUNT_NUMBER FROM IFRS_LIFETIME_DETAIL);
