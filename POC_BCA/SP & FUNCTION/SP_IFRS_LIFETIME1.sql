CREATE OR REPLACE PROCEDURE SP_IFRS_LIFETIME1
AS
V_CURRDATE DATE;
V_PREVDATE DATE;
...
BEGIN
    ...
    DELETE IFRS_LIFETIME_DETAIL_UNP WHERE DOWNLOAD_DATE >= V_CURRDATE;
    ...
    A.LOAN_DUE_DATE,
    A.LOAN_START_DATE,
    CASE WHEN RESERVED_DATE_7 IS NOT NULL AND NPL_DATE IS NOT NULL THEN ((12 + CEIL(MONTHS_BETWEEN (NPL_DATE, RESERVED_DATE_7))))
                                                 ELSE 12 END LIFETIME,
    (A.RESERVED_VARCHAR_2)GOL_DEB,
    C.SEGMENTATION_ID,
    A.REVOLVING_FLAG,
    ...
    WHERE A.DOWNLOAD_DATE = V_CURRDATE
          --AND A.ACCOUNT_STATUS = 'C'
          AND A.REVOLVING_FLAG = 1
          AND (NPL_DATE IS NOT NULL OR BI_COLLECTABILITY >= '3')
          AND A.DATA_SOURCE = 'CRD';
          COMMIT;

          /*UPDATE IFRS_LIFETIME_DETAIL
            SET LIFETIME_PERIOD = CASE WHEN REVOLVING_FLAG = '1' THEN CASE WHEN LIFETIME_PERIOD < 0 THEN 12 ELSE LIFETIME_PERIOD END
          ...
          */
          ...
          INSERT INTO IFRS_LIFETIME_DETAIL_UNP
          SELECT * FROM IFRS_LIFETIME_DETAIL WHERE REVOLVING_FLAG = '0' AND MONTHS_BETWEEN (LOAN_DUE_DATE, LOAN_START_DATE) < 0
                                                AND DOWNLOAD_DATE = V_CURRDATE;COMMIT;
          ...
          DELETE IFRS_LIFETIME_DETAIL
          WHERE REVOLVING_FLAG = '0' AND MONTHS_BETWEEN (LOAN_DUE_DATE, LOAN_START_DATE) < 0
                                     AND DOWNLOAD_DATE = V_CURRDATE;COMMIT;
