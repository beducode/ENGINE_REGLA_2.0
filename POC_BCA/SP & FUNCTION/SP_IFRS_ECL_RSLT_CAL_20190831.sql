CREATE OR REPLACE PROCEDURE SP_IFRS_ECL_RSLT_CAL_20190831(v_ECLID NUMBER DEFAULT(0), v_DOWNLOADDATE DATE DEFAULT('1-JAN-1900'))
AS
    ...
    IF v_CALCMODE = 'YEAR' THEN
        ...
        ******************************************************/
        ...
        SELECT
            ...
            B.BUCKET_ID,
            A.CR_STAGE,
            CASE WHEN A.CR_STAGE = '1' THEN NVL(L.LIFETIME_PERIOD,12) ELSE LEAST(NVL(C.MAX_COUNTER,120), NVL(L.LIFETIME_PERIOD,120)) END AS LIFETIME_PERIOD,
            --LEAST(NVL(C.MAX_COUNTER,120), NVL(L.LIFETIME_PERIOD,120)) AS LIFETIME_PERIOD,
            B.FL_YEAR,
            B.FL_MONTH,
            B.FL_SEQ,
            CASE WHEN A.CR_STAGE = '1' AND NVL(A.REVOLVING_FLAG,0) = 1 THEN NVL(B.PD,0)
            ...
            CASE WHEN B.FL_YEAR = 1 OR (A.DATA_SOURCE IN ('BTRD', 'RKN')) THEN 1 ELSE (1/POWER(1+NVL(A.EIR, A.INTEREST_RATE)/100, B.FL_YEAR - 1)) END AS DISCOUNT_RATE,
            ...
            C.CCF_RATE,
            C.CCF_AMOUNT,
            C.EAD_AMOUNT * (CASE WHEN B.FL_YEAR = 1 OR (A.DATA_SOURCE IN ('BTRD', 'RKN')) THEN 1 ELSE (1/POWER(1+NVL(A.EIR, A.INTEREST_RATE)/100, B.FL_YEAR - 1)) END)
            * CASE WHEN A.CR_STAGE = '1' THEN NVL(B.PD,0)
                   WHEN A.CR_STAGE = '3' AND B.FL_YEAR = 1 THEN 1
                   WHEN A.CR_STAGE = '3' AND B.FL_YEAR > 1 THEN 0
                   WHEN B.FL_YEAR = CEIL( LEAST((NVL(C.MAX_COUNTER,120)/12), (NVL(L.LIFETIME_PERIOD,120)/12)) ) THEN
                        NVL(B.PD,0)
         ...
         INNER JOIN TMP_IFRS_EAD_RESULT C
           ...
           AND B.FL_YEAR <= CASE WHEN A.CR_STAGE = 1 OR A.CR_STAGE IS NULL THEN 1 ELSE CEIL( LEAST((NVL(C.MAX_COUNTER,120)/12), (NVL(L.LIFETIME_PERIOD,120)/12)) ) END
--           AND MOD(C.COUNTER,12) = 1
         LEFT JOIN IFRS_LGD_TERM_STRUCTURE D
          ...
          AND D.FL_YEAR <= CASE WHEN A.CR_STAGE = 1 OR A.CR_STAGE IS NULL THEN 1 ELSE CEIL( LEAST((NVL(C.MAX_COUNTER,120)/12), (NVL(L.LIFETIME_PERIOD,120)/12)) ) END
    ...
    ELSE
        ...
        WITH TMP_PD_TERM_STRUCTURE AS (
            SELECT
            ROW_NUMBER() OVER (PARTITION BY EFF_DATE, PD_RULE_ID, TM_TYPE, FL_SEQ ORDER BY BUCKET_ID) AS RN,
            SUM(1) OVER (PARTITION BY EFF_DATE, PD_RULE_ID, TM_TYPE, FL_SEQ) AS MAX_BUCKET,
            TS.*
            FROM IFRS_PD_TERM_STRUCTURE TS
            WHERE TM_TYPE = 'MONTH'
            AND EXISTS
        ...
        SELECT
            ...
            B.FL_MONTH,
            B.FL_SEQ,
            CASE WHEN A.CR_STAGE = '3' THEN 1 ELSE B.OVERRIDE_PD END   AS PD_RATE,
            D.OVERRIDE_LGD  AS LGD_RATE,
            (1/POWER(1+NVL(A.EIR, A.INTEREST_RATE)/100/12, B.FL_SEQ)) AS DISCOUNT_RATE,
            C.COUNTER       AS COUNTER_PAYSCHD,
            C.EAD_AMOUNT,
            C.FAIRVALUE,
            C.INTEREST_ACCRUED,
            ...
            C.CCF_RATE,
            C.CCF_AMOUNT,
            C.EAD_AMOUNT * (1/POWER(1+NVL(A.EIR, A.INTEREST_RATE)/100/12, B.FL_SEQ)) * CASE WHEN A.CR_STAGE = '3' THEN 1 ELSE B.OVERRIDE_PD END * NVL(D.OVERRIDE_LGD,1) AS ECL_AMOUNT
         ...
         LEFT JOIN IFRS_LIFETIME_HEADER L
           ...
           AND L.DOWNLOAD_DATE = G.LT_EFF_DATE
         INNER JOIN TMP_PD_TERM_STRUCTURE B
            ON G.PD_MODEL_ID = B.PD_RULE_ID
           AND B.BUCKET_ID = A.BUCKET_ID
           AND B.EFF_DATE = G.PD_EFF_DATE --YTA: PERLU DIUPDATE DENGAN LOGIC EFFECTIVE DATE HASIL ECL CONFIG
           AND B.FL_SEQ < CASE WHEN A.CR_STAGE = 1 OR A.CR_STAGE IS NULL THEN 12 ELSE NVL(L.LIFETIME_PERIOD,120) END
         INNER JOIN IFRS_EAD_RESULT C
            ON A.MASTERID = C.MASTERID
           AND A.DOWNLOAD_DATE = C.DOWNLOAD_DATE
           AND C.ECL_MODEL_ID = G.ECL_MODEL_ID
           AND C.COUNTER =  B.FL_SEQ + 1
          --AND B.FL_SEQ <= ISNULL(A.LIFETIME,60)
         LEFT JOIN IFRS_LGD_TERM_STRUCTURE D
           ON G.LGD_MODEL_ID = D.LGD_RULE_ID
          AND D.EFF_DATE = G.LGD_EFF_DATE --YTA: PERLU DIUPDATE DENGAN LOGIC EFFECTIVE DATE HASIL ECL CONFIG
          AND B.FL_SEQ = D.FL_SEQ
          AND D.FL_SEQ < CASE WHEN A.CR_STAGE = 1 OR A.CR_STAGE IS NULL THEN 12 ELSE NVL(L.LIFETIME_PERIOD,120) END
         WHERE A.DOWNLOAD_DATE = v_DOWNLOADDATE
          AND G.ECL_MODEL_ID = v_ECLID;
    ...
    INSERT INTO IFRS_ECL_RESULT_DETAIL(
     ...
     (
        SELECT
            ...
            SUM(NVL(PREPAYMENT_AMOUNT,0))  AS PREPAYMENT_AMOUNT,
