CREATE OR REPLACE PROCEDURE SP_IFRS_IMPI_JOURNAL_DATA_RAL (
...
--  COMMIT;
  ...
  INSERT INTO IFRS_IMP_JOURNAL_DATA_RAL
    ...
    SELECT V_MAXCURR,
           ...
           C.DRCR,
           CASE
             WHEN C.JOURNALCODE = 'BKIUW' THEN
              round(NVL(IA_UNWINDING_AMOUNT, 0),2)
             WHEN C.JOURNALCODE = 'BKIPP' THEN
             --  NVL (ECL_AMOUNT, 0)
              0
           END,
           CASE
             WHEN C.JOURNALCODE = 'BKIUW' THEN
              round(NVL(IA_UNWINDING_AMOUNT, 0) * NVL(B.EXCHANGE_RATE, 1),2)
             WHEN C.JOURNALCODE = 'BKIPP' THEN
  ...
  INSERT INTO IFRS_IMP_JOURNAL_DATA_RAL
    ...
    SELECT V_MAXCURR,
           ...
           C.DRCR,
           CASE
             WHEN UPPER(C.JOURNALCODE) = 'IRBS' THEN
              round(NVL(IMA.INTEREST_ACCRUED, 0),2)
           END,
           CASE
             WHEN UPPER(C.JOURNALCODE) = 'IRBS' THEN
              round(NVL(IMA.INTEREST_ACCRUED, 0) * NVL(IMA.EXCHANGE_RATE, 1),2)
           END,
           C.GL_NO,
           C.GL_INTERNAL_CODE,
           C.GL_CONSTNAME,
