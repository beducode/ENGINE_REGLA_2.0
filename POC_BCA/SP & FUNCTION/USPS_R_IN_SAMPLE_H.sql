CREATE OR REPLACE PROCEDURE  USPS_R_IN_SAMPLE_H (
    v_MODEL_ID    NUMBER,
    v_MODEL_SEQ   NUMBER,
...
BEGIN
    ...
    IF v_status = 1 THEN
        ...
        A.IN_SAMPLE_START_DATE || ' to ' || A.IN_SAMPLE_END_DATE InSamplePeriod
        FROM IFRS_FL_MODEL_VAR A
        JOIN R_BACKTEST_IN_SAMPLE_DTL B
    ...
    ELSE
        ...
        A.IN_SAMPLE_START_DATE || ' to ' || A.IN_SAMPLE_END_DATE InSamplePeriod
        FROM IFRS_FL_MODEL_VAR A
        JOIN R_BACKTEST_IN_SAMPLE_DTL_PEN B
            ON A.PKID = v_MODEL_ID
            AND A.PKID = B.MODEL_ID
            AND B.MODEL_SEQ = v_MODEL_SEQ
        LEFT JOIN IFRS_PD_RULES_CONFIG PD
