CREATE OR REPLACE PROCEDURE SP_IFRS_PD_MAA_LOGIT_ODR
AS
    v_CURRDATE DATE;
BEGIN
    ...
    SELECT A.EFF_DATE,
        A.BASE_DATE,
        A.PD_RULE_ID,
        CASE WHEN A.ODR > 0 THEN LN(A.ODR / (1-A.ODR)) ELSE 0 END,
        0
    FROM IFRS_PD_MAA_ODR A
    ...
    MERGE INTO IFRS_PD_MAA_LOGIT_ODR A
    USING
    (
        SELECT PD_RULE_ID,
            LOGIT_ODR
        FROM IFRS_PD_MAA_LOGIT_ODR
        WHERE EFF_DATE = ADD_MONTHS(v_CURRDATE, -12)
    ) B
    ON (A.EFF_DATE = v_CURRDATE AND A.PD_RULE_ID = B.PD_RULE_ID)
    WHEN MATCHED THEN
    UPDATE SET
