CREATE OR REPLACE PROCEDURE SP_IFRS_NOMINATIVE_RKN_W (
...
BEGIN
    ...
    IF v_RUNNING_MODE = 0 THEN
        DELETE /*+ PARALLEL(8) */ IFRS.IFRS_NOMINATIVE WHERE REPORT_DATE = V_CURRDATE AND DATA_SOURCE = 'RKN';
    ELSE
        DELETE /*+ PARALLEL(8) */ IFRS.IFRS_NOMINATIVE WHERE REPORT_DATE = V_CURRDATE AND DATA_SOURCE = 'RKN'
    ...
    INSERT /*+ PARALLEL(8) */ INTO IFRS.IFRS_NOMINATIVE (MASTERID,
      ...
      SELECT /*+ PARALLEL(8) */ A.MASTERID                                                                       AS MASTERID,
             ...
             CASE WHEN A.REVOLVING_FLAG = 1 THEN 'Y'
             ...
             A.BRANCH_CODE                                                                    AS BRANCH_CODE,
                -- 600084535 - Khusus CIS 104014030000 maka
                -- lbu form = 22
                -- norek LBU = 800700000242
             case when A.CUSTOMER_NUMBER='104014030000' then '22'
                 else B.RESERVED_VARCHAR_1 end                                                AS LBU_FORM,
             case when A.CUSTOMER_NUMBER='104014030000' then
                    '800700000242'
                 else
                    case when A.MASTER_ACCOUNT_CODE LIKE 'GL%' then
                        SUBSTR (A.ACCOUNT_NUMBER, 1, 5) || A.CURRENCY || A.BRANCH_CODE
                    else
                        SUBSTR (A.ACCOUNT_NUMBER, 2, 6) || A.CURRENCY
                    end
                 end
