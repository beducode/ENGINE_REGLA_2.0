CREATE OR REPLACE PROCEDURE  USPS_PD_CUMMULATIVE
(
    v_eff_date    date,
    v_pd_rule_id  number,
...
BEGIN
        ...
        v_query := 'SELECT PD_RULE_NAME AS "PD Model", EFF_DATE AS "Period", BUCKET_ID AS "Bucket ID", BUCKET_NAME AS "Bucket Name", ' || v_columnAlias || ' FROM
                    (
                        SELECT
                            B.PD_RULE_NAME, A.EFF_DATE, A.BUCKET_ID, C.BUCKET_NAME, FL_YEAR, round(CUMULATIVE_PD,6) CUMULATIVE_PD
