CREATE OR REPLACE PROCEDURE SP_IFRS_UPDATE_CCF_DAY1
AS
  V_CURRDATE  date;
...
BEGIN
  ...
  LOOP
    ...
    V_STR_SQL := '  UPDATE IFRS_MASTER_ACCOUNT_MONTHLY A
      SET  A.CCF_RULE_ID =(SELECT PKID FROM ifrs_ccf_rules_config WHERE segmentation_id = ' || V_RULE_ID || ' and AVERAGE_METHOD = ''Simple''),
            A.CCF_SEGMENT =  ''' || V_Sub_Segment || '''
