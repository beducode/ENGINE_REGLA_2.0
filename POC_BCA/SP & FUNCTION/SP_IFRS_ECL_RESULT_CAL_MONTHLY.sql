CREATE OR REPLACE PROCEDURE SP_IFRS_ECL_RESULT_CAL_MONTHLY(v_ECLID NUMBER DEFAULT(0), v_DOWNLOADDATE DATE DEFAULT('1-JAN-1900'))
AS
...
BEGIN
    ...
    A.BUCKET_GROUP,
    A.BUCKET_ID,
    (A.FL_YEAR - 1) * 12 + B.MONTHS FL_SEQ,
    A.FL_YEAR,
    ADD_MONTHS(A.EFF_DATE, (A.FL_YEAR - 1) * 12 + B.MONTHS - 1) FL_DATE,
    B.MONTHS AS FL_MONTH,
    A.OVERRIDE_PD / 12 AS PD
    ...
    AND EXISTS
    (
        SELECT 1 FROM IFRS_ECL_MODEL_CONFIG C
        WHERE A.PD_RULE_ID = C.PD_MODEL_ID
        AND C.ECL_MODEL_ID = v_ECLID AND C.DOWNLOAD_DATE = v_DOWNLOADDATE AND C.PD_EFF_DATE = A.EFF_DATE
        AND A.MODEL_ID = CASE WHEN C.PD_FL_FLAG = 1 THEN C.PD_FL_MODEL ELSE 0  END   --MODEL_ID '0' = TTC PD = NO FORWARD LOOKING
    )
    UNION ALL
    ...
    A.BUCKET_GROUP,
    C.BUCKET_ID,
    (A.FL_YEAR - 1) * 12 + D.MONTHS FL_SEQ,
    A.FL_YEAR,
    ADD_MONTHS(A.EFF_DATE, (A.FL_YEAR - 1) * 12 + D.MONTHS - 1) FL_DATE,
    D.MONTHS AS FL_MONTH,
    A.OVERRIDE_PD / 12 AS PD
    FROM IFRS_PD_TERM_STRUCTURE A
    ...
    /*****************************************************
    CALCULATE ECL AS MONTHLY FOR FULL YEAR
    ******************************************************/
    ...
    SELECT
        ...
        B.BUCKET_ID,
        A.CR_STAGE,
        CASE WHEN (A.CR_STAGE = '1' AND NVL(A.REVOLVING_FLAG,0) = 1) OR A.DATA_SOURCE = 'LIMIT' THEN
           12
        ELSE
        ...
        B.FL_MONTH,
        B.FL_SEQ,
        CASE WHEN (A.CR_STAGE = '1' AND NVL(A.REVOLVING_FLAG,0) = 1) OR A.DATA_SOURCE = 'LIMIT' THEN NVL(B.PD,0)
             WHEN A.CR_STAGE = '3' AND B.FL_SEQ = 1 THEN 1
             WHEN A.CR_STAGE = '3' AND B.FL_SEQ > 1 THEN 0
             ELSE NVL(B.PD,0)
        END AS PD_RATE,
        D.OVERRIDE_LGD  AS LGD_RATE,
        CASE WHEN B.FL_SEQ = 1 THEN 1
             WHEN A.EIR < 0 THEN (1/POWER(1+NVL(A.INTEREST_RATE,0)/100/12, B.FL_SEQ - 1))
             ELSE (1/POWER(1+NVL(A.EIR, A.INTEREST_RATE)/100/12, B.FL_SEQ - 1)) END AS DISCOUNT_RATE,
        ...
        * CASE WHEN B.FL_SEQ = 1 THEN 1
               WHEN A.EIR < 0 THEN (1/POWER(1+NVL(A.INTEREST_RATE,0)/100/12, B.FL_SEQ - 1))
               ELSE (1/POWER(1+NVL(A.EIR, A.INTEREST_RATE)/100/12, B.FL_SEQ - 1)) END
        * CASE WHEN (A.CR_STAGE = '1' AND NVL(A.REVOLVING_FLAG,0) = 1) OR A.DATA_SOURCE = 'LIMIT' THEN NVL(B.PD,0)
               WHEN A.CR_STAGE = '3' AND B.FL_SEQ = 1 THEN 1
               WHEN A.CR_STAGE = '3' AND B.FL_SEQ > 1 THEN 0
               ELSE NVL(B.PD,0)
     ...
     INNER JOIN TMP_IFRS_EAD_RESULT C
       ...
       AND C.COUNTER <= LEAST(NVL(C.MAX_COUNTER,12), NVL(L.LIFETIME_PERIOD,12))
       AND B.FL_SEQ = C.COUNTER
