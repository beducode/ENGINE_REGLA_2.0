CREATE OR REPLACE PROCEDURE SP_IFRS_EXEC_AND_LOG_PROCESS (
   P_SP_NAME       VARCHAR2,
...
BEGIN
    ...
    *******************************************************************************/
      ...
      SET A.END_DATE = SYSTIMESTAMP,
          ISCOMPLETE = 'Y',
          PRC_PROCESS_TIME = TO_CHAR(EXTRACT(HOUR FROM (SYSTIMESTAMP - START_DATE))) || ':' ||
                             TO_CHAR(EXTRACT(MINUTE FROM (SYSTIMESTAMP - START_DATE))) || ':' ||
                             TO_CHAR(EXTRACT(SECOND FROM (SYSTIMESTAMP - START_DATE))),
          REMARK = 'Successed'
...
--      SET SESSION_PROCESS_TIME =REPLACE (REPLACE (SYSTIMESTAMP - V_MINSTARTDATESESSION,'.000000000',''),'000000000','')
