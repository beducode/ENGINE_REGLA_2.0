CREATE OR REPLACE PROCEDURE SP_IFRS_LI_ACCT_EIR_ECF_MAIN
AS
  V_CURRDATE DATE;
  V_PREVDATE DATE;
  V_VMIN_ID NUMBER(19);
...
BEGIN
    ...
    VALUES (V_CURRDATE,SYSTIMESTAMP,'START','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','');

    COMMIT;

    --RESET DATA BEFORE PROCESSING
    DELETE FROM IFRS_LI_ACCT_EIR_ACCRU_PREV
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_T7';
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_LI_ACCT_EIR_CF_ECF';
    ...
    LEFT JOIN IFRS_LI_ACCT_COST_FEE C ON C.DOWNLOAD_DATE = V_CURRDATE
        ...
        AND C.CF_ID NOT IN (SELECT CF_ID FROM IFRS_LI_ACCT_COST_FEE WHERE DOWNLOAD_DATE = V_CURRDATE AND FLAG_REVERSE = 'Y' AND CF_ID_REV IS NOT NULL
                            UNION ALL
    ...
    -- SISA UNAMORT
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_T10';
    ...
    LEFT JOIN (SELECT DISTINCT MASTERID FROM IFRS_LI_ACCT_EIR_STOP_REV WHERE DOWNLOAD_DATE = V_CURRDATE) A ON A.MASTERID = B.MASTERID
    --20180116 RESONA REQ
    LEFT JOIN TMP_LI_TODAYREV CFREV ON CFREV.MASTERID = B.MASTERID
    WHERE B.DOWNLOAD_DATE IN (V_CURRDATE,V_PREVDATE)
        AND B.STATUS = 'ACT'
        AND A.MASTERID IS NULL
        --20180116 EXCLUDE CF REVERSAL AND ITS PAIR
        AND B.CF_ID NOT IN (SELECT CF_ID FROM IFRS_LI_ACCT_COST_FEE WHERE DOWNLOAD_DATE = V_CURRDATE AND FLAG_REVERSE = 'Y' AND CF_ID_REV IS NOT NULL
                            UNION ALL
                            SELECT CF_ID_REV FROM IFRS_LI_ACCT_COST_FEE WHERE DOWNLOAD_DATE = V_CURRDATE AND FLAG_REVERSE = 'Y' AND CF_ID_REV IS NOT NULL)
    ...
    MERGE INTO IFRS_LI_ACCT_EIR_CF_ECF A
    USING TMP_LI_T10 B
    ON (B.MASTERID = A.MASTERID
       )
    WHEN MATCHED THEN
    UPDATE
    SET  FEE_AMT = A.FEE_AMT + B.FEE_AMT
    ...
    THEN
        ...
        INSERT INTO TMP_LI_T1 (MASTERID,ACCTNO)
        ...
        MERGE INTO IFRS_LI_ACCT_EIR_CF_ECF A
        USING (SELECT * FROM IFRS_LI_ACCT_EIR_ACF
               WHERE ID IN (SELECT ID FROM TMP_LI_P1)
               ) B
        ON (A.MASTERID = B.MASTERID
            --20160407 EIR STOP REV
            AND A.MASTERID NOT IN (SELECT MASTERID FROM IFRS_LI_ACCT_EIR_STOP_REV WHERE DOWNLOAD_DATE = V_CURRDATE)
            ----ADD 20180924
        ...
        ON ((B.MASTERID = A.MASTERID)
            --20180404 ADD FILTER
            AND A.MASTERID IN (SELECT MASTERID FROM TMP_LI_T3)
    ...
    INSERT INTO TMP_LI_T10 (MASTERID,FEE_AMT,COST_AMT)
    SELECT B.MASTERID
    ...
    -- UPDATE PREV EIR
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_T13';

    INSERT INTO TMP_LI_T13 (MASTERID,N_EFF_INT_RATE,ENDAMORTDATE)
    SELECT B.MASTERID
        ,B.N_EFF_INT_RATE
        ,B.ENDAMORTDATE
    ...
    WHERE MASTERID IN (SELECT MASTERID
        ...
        AND MASTERID NOT IN (SELECT MASTERID FROM IFRS_LI_EVENT_CHANGES WHERE EVENT_ID IN (0,1,2,3) AND EFFECTIVE_DATE = V_CURRDATE);
    ...
    SET STATUS = 'PNL'
        ,CREATEDBY = 'EIRECF1'
    WHERE DOWNLOAD_DATE = V_CURRDATE
        AND MASTERID IN (SELECT MASTERID FROM IFRS_LI_ACCT_EIR_CF_ECF WHERE TOTAL_AMT = 0 OR TOTAL_AMT_ACRU = 0)
        AND STATUS = 'ACT'
        --20180116 EXCLUDE CF REV AND ITS PAIR, WILL BE HANDLED BY ACF_ABN
        AND CF_ID NOT IN (SELECT CF_ID FROM IFRS_LI_ACCT_COST_FEE WHERE DOWNLOAD_DATE = V_CURRDATE AND FLAG_REVERSE = 'Y' AND CF_ID_REV IS NOT NULL
                          UNION ALL
                          SELECT CF_ID_REV FROM IFRS_LI_ACCT_COST_FEE WHERE DOWNLOAD_DATE = V_CURRDATE AND FLAG_REVERSE = 'Y' AND CF_ID_REV IS NOT NULL);

    -- IF LAST COST FEE PREV IS CURRDATE
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_T11';
    ...
    WHERE B.MASTERID IN (SELECT MASTERID FROM IFRS_LI_ACCT_EIR_CF_ECF WHERE TOTAL_AMT = 0 OR TOTAL_AMT_ACRU = 0);

    COMMIT;


    MERGE INTO IFRS_LI_ACCT_EIR_COST_FEE_PREV A
    USING TMP_LI_T11 B
    ON (A.DOWNLOAD_DATE = B.CURRDATE
        ...
        AND A.CF_ID NOT IN (SELECT CF_ID FROM IFRS_LI_ACCT_COST_FEE WHERE DOWNLOAD_DATE = V_CURRDATE AND FLAG_REVERSE = 'Y' AND CF_ID_REV IS NOT NULL
                            UNION ALL
                            SELECT CF_ID_REV FROM IFRS_LI_ACCT_COST_FEE WHERE DOWNLOAD_DATE = V_CURRDATE AND FLAG_REVERSE = 'Y' AND CF_ID_REV IS NOT NULL)
       )
    WHEN MATCHED THEN
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_T12';
    ...
    WHEN MATCHED THEN
    UPDATE
    SET STATUS = CASE WHEN STATUS = 'ACT' THEN 'PNL2' ELSE STATUS END
        ,CREATEDBY = 'EIRECF2';
    ...
    THEN
        ...
        SELECT A.FACNO
            ...
            ,ROUND(CAST(CAST(CASE WHEN B.FLAG_REVERSE = 'Y'THEN - 1 * B.AMOUNT ELSE B.AMOUNT
                            END AS BINARY_DOUBLE) / NULLIF(CAST(C.SUM_AMT AS BINARY_DOUBLE), 0) AS NUMBER(32, 20)) * A.N_ACCRU_FEE, V_ROUND) AS N_AMOUNT
            ,B.STATUS
            ,SYSTIMESTAMP
            ,A.ACCTNO
            ,A.MASTERID
        ...
        SELECT A.FACNO
            ...
            ,B.CCY
            ,ROUND(CAST(CAST(CASE WHEN B.FLAG_REVERSE = 'Y' THEN - 1 * B.AMOUNT ELSE B.AMOUNT
                            END AS BINARY_DOUBLE) / NULLIF(CAST(C.SUM_AMT AS BINARY_DOUBLE), 0) AS NUMBER(32, 20)) * A.N_ACCRU_COST, V_ROUND)  AS N_AMOUNT
            ,B.STATUS
            ,SYSTIMESTAMP
            ,A.ACCTNO
            ,A.MASTERID
    ...
    WHERE MASTERID IN (SELECT MASTERID FROM IFRS_LI_ACCT_EIR_CF_ECF)
        AND AMORTSTOPDATE IS NULL;

    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','2');

    COMMIT;*
    -- INSERT FROM IFRS_LI_PAYM_SCHD FILTERED
    -- REMARKS DAHULU KARENA SUDAH DI INSERT DI SP_IFRS_LI_PAYMENT_SCHEDULE 20160524
    TRUNCATE TABLE IFRS_LI_PAYM_CORE_SRC
...
-- CALC EFF RATE FROM TABLE IFRS_LI_PAYM_CORE
...
-- END GET LAST OR START DATE FOR ASSIGN FIRST PAYM DATE  --RIDWAN
    ...
    INSERT  INTO TT_PSAK_LAST_PAYM_DATE
            ( MASTERID ,
              CURRDATE ,
...
--UPDATE DISB AMOUNT 20160428
    ...
    MERGE INTO IFRS_LI_PAYM_CORE_SRC A
        USING
...
--REMARKS DULU 20160524
    ...
    WHERE PREV_PMT_DATE = PMT_DATE
        AND MASTERID IN (SELECT MASTERID
                         FROM IFRS_LI_ACCT_EIR_CF_ECF
                        WHERE ((TOTAL_AMT <> 0 AND TOTAL_AMT_ACRU <> 0)
                                OR STAFFLOAN = 1
                                --20170927, ANYINK
                                OR MASTERID IN (SELECT DISTINCT MASTERID FROM IFRS_LI_EVENT_CHANGES WHERE DOWNLOAD_DATE = V_CURRDATE AND EVENT_ID = 4)
                                )
                        );

    COMMIT;

    EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_LI_GS_MASTERID';
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_LI_ACCT_EIR_PAYM';
    ...
    LOOP --LOOP
        INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
        VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_PAYM_CORE_PROCESS',TO_CHAR(V_VX));

        EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_LI_PAYM_CORE';
        ...
        /*  BCA DISABLE BPI ,SPECIAL_FLAG --- BPI FLAG ONLY CTBC */
        FROM IFRS_LI_PAYM_CORE_SRC
        WHERE MASTERID IN (SELECT MASTERID FROM IFRS_LI_GS_MASTERID WHERE ID >= V_VX AND ID < (V_VX + V_VX_INC));

        SP_IFRS_LI_EXEC_AND_LOG ('SP_IFRS_LI_PAYM_CORE_PROC_NOP'); -- TANPA EFEKTIFISASI
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','3');

    -- UPDATE NPV RATE FOR STAFF LOAN
    MERGE INTO IFRS_LI_ACCT_EIR_PAYM A
    USING IFRS_LI_ACCT_EIR_CF_ECF B
    ON ( B.TOTAL_AMT = 0
    ...
    SET NPV_INSTALLMENT = CASE
            WHEN ROUND(FN_LI_CNT_DAYS_30_360(STARTAMORTDATE, PMT_DATE) / 30, 0) = 0
                THEN N_INSTALLMENT / (POWER(1 + NVL(NPV_RATE, 0) / 360 / 100, FN_LI_CNT_DAYS_30_360(STARTAMORTDATE, PMT_DATE)))
            ELSE N_INSTALLMENT / NULLIF((POWER(1 + NVL(NPV_RATE, 0) / 12 / 100, ROUND(FN_LI_CNT_DAYS_30_360(STARTAMORTDATE, PMT_DATE) / 30, 0))), 0)
            END
    WHERE NPV_RATE > 0;
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_B1';

    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_B2';

    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_LI_B3';

    -- GET OS
    INSERT INTO TMP_LI_B1 (MASTERID,N_OSPRN)
    ...
    --GET NPV SUM
    INSERT INTO TMP_LI_B2 (MASTERID,NPV_SUM)
    SELECT MASTERID
    ...
    -- UPDATE BACK
    MERGE INTO IFRS_LI_ACCT_EIR_CF_ECF B
    USING TMP_LI_B3 A
    ON ( A.MASTERID = B.MASTERID
       )
    WHEN MATCHED THEN
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','3A');
    ...
    THEN
        ...
        INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
        VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','3B');
        ...
        SELECT A.FACNO
            ...
            ,B.CCY
            ,ROUND(CAST(CAST(CASE WHEN B.FLAG_REVERSE = 'Y' THEN - 1 * B.AMOUNT ELSE B.AMOUNT
                            END AS BINARY_DOUBLE) / NULLIF(CAST(C.SUM_AMT AS BINARY_DOUBLE), 0) AS NUMBER(32, 20)) * A.N_ACCRU_FEE * - 1, V_ROUND) AS N_AMOUNT
            ,B.STATUS
            ,SYSTIMESTAMP
            ,A.ACCTNO
            ,A.MASTERID
        ...
        JOIN TMP_LI_TF C ON C.DOWNLOAD_DATE = A.ECFDATE
            AND C.MASTERID = A.MASTERID
        --20160407 EIR STOP REV
        LEFT JOIN (SELECT DISTINCT MASTERID FROM IFRS_LI_ACCT_EIR_STOP_REV WHERE DOWNLOAD_DATE = V_CURRDATE) D ON A.MASTERID = D.MASTERID
        WHERE A.ID IN (SELECT ID FROM TMP_LI_P1)
            --20160407 EIR STOP REV
        ...
        INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
        VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','3C');
        ...
        SELECT A.FACNO
            ...
            ,B.CCY
            ,ROUND(CAST(CAST(CASE WHEN B.FLAG_REVERSE = 'Y' THEN - 1 * B.AMOUNT ELSE B.AMOUNT
                            END AS BINARY_DOUBLE) / NULLIF(CAST(C.SUM_AMT AS BINARY_DOUBLE), 0) AS NUMBER(32, 20)) * A.N_ACCRU_COST * - 1, V_ROUND) AS N_AMOUNT
            ,B.STATUS
            ,SYSTIMESTAMP
            ,A.ACCTNO
            ,A.MASTERID
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','3D');

    COMMIT;

    -- 20160412 GROUP MULTIPLE ROWS BY CF_ID
    SP_IFRS_LI_EXEC_AND_LOG ('SP_IFRS_LI_ACCT_EIR_CF_ECF_GRP');

    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','4 GS START');

    COMMIT;

    DELETE FROM IFRS_LI_ACCT_EIR_FAILED_GS
    WHERE DOWNLOAD_DATE = V_CURRDATE;

    DELETE FROM IFRS_LI_ACCT_EIR_GS_RESULT
    WHERE DOWNLOAD_DATE = V_CURRDATE;

    EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_LI_ACCT_EIR_CF_ECF1';
    ...
    WHERE (B.TOTAL_AMT <> 0 AND B.TOTAL_AMT_ACRU <> 0)
        ...
        --20170927, IVAN NOCF
        OR (B.MASTERID IN (SELECT DISTINCT MASTERID FROM IFRS_LI_EVENT_CHANGES WHERE DOWNLOAD_DATE = V_CURRDATE AND EVENT_ID = 4));
    ...
    --CLEAN UP
    DELETE FROM IFRS_LI_ACCT_EIR_GS_RESULT3
    WHERE DOWNLOAD_DATE = V_CURRDATE;

    DELETE FROM IFRS_LI_ACCT_EIR_FAILED_GS3
    WHERE DOWNLOAD_DATE = V_CURRDATE;
    ...
    INSERT INTO IFRS_LI_GS_MASTERID (MASTERID)
    SELECT A.MASTERID
    FROM (SELECT MASTERID,PERIOD
          FROM IFRS_LI_ACCT_EIR_PAYM
          WHERE PREV_PMT_DATE = PMT_DATE
              AND MASTERID IN (SELECT MASTERID FROM IFRS_LI_ACCT_EIR_CF_ECF1
                                WHERE (STAFFLOAN = 1 AND PREV_EIR IS NULL)
    ...
    LOOP --LOOP
        INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
        VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_GS_RANGE',TO_CHAR(V_VX));

        COMMIT;

        V_ID2 := V_VX + V_VX_INC - 1;

        SP_IFRS_LI_ACCT_EIR_GS_RANGE(V_VX,V_ID2);

        INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
        VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_GS_RANGE','DONE');

        COMMIT;

        SP_IFRS_LI_EXEC_AND_LOG( 'SP_IFRS_LI_ACCT_EIR_GS_PROC3');
    ...
    MERGE INTO IFRS_LI_ACCT_EIR_CF_ECF1 A
    USING IFRS_LI_ACCT_EIR_GS_RESULT3 B
    ON (B.MASTERID = A.MASTERID
    ...
    WHEN MATCHED THEN
    UPDATE
    SET GAIN_LOSS_AMT = ROUND(B.GLOSS, V_ROUND)
        ,GAIN_LOSS_FEE_AMT = CASE WHEN FEE_AMT <> 0 AND COST_AMT = 0 THEN ROUND(B.GLOSS, V_ROUND)
                                  WHEN FEE_AMT = 0 AND COST_AMT <> 0 THEN 0
                                  ELSE ROUND(B.GLOSS * FEE_AMT / TOTAL_AMT, V_ROUND)
                                  END
        ,GAIN_LOSS_COST_AMT = CASE WHEN FEE_AMT = 0 AND COST_AMT <> 0 THEN ROUND(B.GLOSS, V_ROUND)
                                   WHEN FEE_AMT <> 0AND COST_AMT = 0 THEN 0
    ...
    LOOP --LOOP
        ...
        SP_IFRS_LI_EXEC_AND_LOG('SP_IFRS_LI_ACCT_EIR_GS_ALL');
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','5 GS END');

    COMMIT;

    SP_IFRS_LI_EXEC_AND_LOG('SP_IFRS_LI_ACCT_EIR_GS_INSERT4');

    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','6 ECFNOCF INSERT');

    COMMIT;

    SP_IFRS_LI_EXEC_AND_LOG('SP_IFRS_LI_ACCT_EIR_ECF_ALIGN4');

    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','7 ECFNOCF ALIGNED');
...
--UPDATE PNL IF FAILED GOAL SEEK 20160524
    ...
    WHERE   DOWNLOAD_DATE = @V_CURRDATE
            ...
            AND STATUS = 'ACT'
*/
    SP_IFRS_LI_EXEC_AND_LOG( 'SP_IFRS_LI_ACCT_EIR_GS_INSERT');

    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','8 ECF INSERTED');

    COMMIT;

    SP_IFRS_LI_EXEC_AND_LOG ('SP_IFRS_LI_ACCT_EIR_ECF_ALIGN');

    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','9 ECF ALIGNED');

    COMMIT;

    -- MERGE ECF FOR MASTERID WITH DIFFERENT INTEREST STRUCTURE
    SP_IFRS_LI_EXEC_AND_LOG( 'SP_IFRS_LI_ACCT_EIR_ECF_MERGE');

    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'DEBUG','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','10 ECF MERGED');
    ...
    DELETE FROM IFRS_LI_ACCT_EIR_GAIN_LOSS
    WHERE DOWNLOAD_DATE = V_CURRDATE;
    ...
    USING IFRS_LI_ACCT_EIR_CF_ECF A
    ON (A.MASTERID = Z.MASTERID
        AND Z.ECFDATE = V_CURRDATE
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'END','SP_IFRS_LI_ACCT_EIR_ECF_MAIN','');
