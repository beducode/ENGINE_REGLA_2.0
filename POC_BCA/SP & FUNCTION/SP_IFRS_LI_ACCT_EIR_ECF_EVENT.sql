CREATE OR REPLACE PROCEDURE  SP_IFRS_LI_ACCT_EIR_ECF_EVENT
AS
  V_CURRDATE DATE;
  V_PREVDATE DATE;
...
BEGIN
    ...
    VALUES (V_CURRDATE,SYSTIMESTAMP,'START','SP_IFRS_LI_ACCT_EIR_ECF_EVENT','');

    COMMIT;

    -- RESET
    --UPDATE PMA SET EIR_STATUS='' WHERE DOWNLOAD_DATE=V_CURRDATE AND EIR_STATUS='Y';
    UPDATE IFRS_LI_IMA_AMORT_CURR
    SET EIR_STATUS = ''
        ,ECF_STATUS = ''
    ...
    DELETE IFRS_LI_EVENT_CHANGES
    WHERE DOWNLOAD_DATE = V_CURRDATE;

    DELETE IFRS_LI_EVENT_CHANGES_DETAILS
    WHERE DOWNLOAD_DATE = V_CURRDATE;

    COMMIT;

    -- GET ACTIVE EIR ECF MASTERID
    EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_LI_ACT_ECF_TMP';

    INSERT INTO IFRS_LI_ACT_ECF_TMP (MASTERID)
    ...
    INSERT INTO IFRS_LI_EVENT_CHANGES (
        DOWNLOAD_DATE
        ,MASTERID
        ,ACCOUNT_NUMBER
        ,EFFECTIVE_DATE
    ...
    SELECT V_CURRDATE
        ...
        ,'SP_IFRS_LI_ACCT_EIR_ECF_EVENT'
    FROM IFRS_LI_IMA_AMORT_CURR A
    INNER JOIN IFRS_LI_ACT_ECF_TMP B ON A.MASTERID = B.MASTERID
    INNER JOIN IFRS_LI_IMA_AMORT_PREV C ON A.MASTERID = C.MASTERID
    ...
    SELECT V_CURRDATE
        ...
        ,'SP_IFRS_LI_ACCT_EIR_ECF_EVENT'
    FROM IFRS_LI_IMA_AMORT_CURR A
    INNER JOIN IFRS_LI_ACCT_COST_FEE B ON A.MASTERID = B.MASTERID
    ...
    SELECT  V_CURRDATE ,
          ...
          'SP_IFRS_LI_ACCT_EIR_ECF_EVENT'
    FROM    IFRS_LI_IMA_AMORT_CURR A
            INNER JOIN IFRS_LI_PRODUCT_PARAM B ON A.DATA_SOURCE = B.DATA_SOURCE
  ...
  -- REPAYMENT EVENT
    ...
    SELECT  @V_CURRDATE ,
          ...
          'SP_IFRS_LI_ACCT_EIR_ECF_EVENT'
    FROM    IFRS_LI_IMA_AMORT_CURR A
            INNER JOIN IFRS_LI_MASTER_ACCOUNT B ON A.MASTERID = B.MASTERID
         AND A.DOWNLOAD_DATE = B.DOWNLOAD_DATE
    AND A.CURRENCY = B.CURRENCY
    WHERE   B.FLAG_REPAYMENT = '1'
      AND   A.DOWNLOAD_DATE = @V_CURRDATE
      AND A.MASTERID IN (
       SELECT MASTERID FROM IFRS_LI_MASTER_ACCOUNT
 ...
 -- DISBURSE REVERSAL EVENT
    ...
    AND A.AMORT_TYPE = 'EIR'
        ...
        INSERT  INTO IFRS_LI_EVENT_CHANGES_DETAILS
    ( DOWNLOAD_DATE ,
    MASTERID ,
    ACCOUNT_NUMBER ,
    BEFORE_VALUE ,
    ...
    SELECT  A.DOWNLOAD_DATE ,
          ...
          'SP_IFRS_LI_ACCT_EIR_ECF_EVENT'
    FROM    IFRS_LI_IMA_AMORT_CURR A
          INNER JOIN IFRS_LI_EVENT_CHANGES B ON A.MASTERID = B.MASTERID
    ...
    SELECT V_CURRDATE
        ...
        ,'SP_IFRS_LI_ACCT_EIR_ECF_EVENT'
    FROM IFRS_LI_IMA_AMORT_CURR A
    INNER JOIN IFRS_LI_ACCT_SWITCH B ON A.DOWNLOAD_DATE = B.DOWNLOAD_DATE
    ...
    SELECT V_CURRDATE
        ...
        ,'SP_IFRS_LI_ACCT_EIR_ECF_EVENT'
    FROM IFRS_LI_IMA_AMORT_CURR A
    INNER JOIN IFRS_LI_TRANSACTION_DAILY B ON A.MASTERID = B.MASTERID
        AND A.DOWNLOAD_DATE = B.DOWNLOAD_DATE
        AND B.TRX_CODE = 'PP'
    WHERE A.DOWNLOAD_DATE = V_CURRDATE
        AND A.ACCOUNT_STATUS = 'A'
    ...
    MERGE INTO IFRS_LI_IMA_AMORT_CURR IMA
    USING IFRS_LI_EVENT_CHANGES RES
    ON (IMA.DOWNLOAD_DATE = V_CURRDATE
        AND RES.EFFECTIVE_DATE = V_CURRDATE
        AND RES.MASTERID = IMA.MASTERID
        AND IMA.MASTERID NOT IN (SELECT DISTINCT MASTERID FROM IFRS_LI_ACCT_CLOSED WHERE DOWNLOAD_DATE = V_CURRDATE)
       )
    WHEN MATCHED THEN
    UPDATE
    SET  EIR_STATUS = 'Y'
    ...
    INSERT INTO IFRS_LI_AMORT_LOG (DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES (V_CURRDATE,SYSTIMESTAMP,'END','SP_IFRS_LI_ACCT_EIR_ECF_EVENT','');
