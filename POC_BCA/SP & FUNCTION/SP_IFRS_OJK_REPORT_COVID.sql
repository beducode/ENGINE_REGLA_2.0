CREATE OR REPLACE PROCEDURE SP_IFRS_OJK_REPORT_COVID(v_DOWNLOADDATECUR  DATE DEFAULT ('1-JAN-1900'),v_DOWNLOADDATEPREV  DATE DEFAULT ('1-JAN-1900'))
                                               AS
  V_CURRDATE DATE;
  V_PREVDATE DATE;
...
BEGIN
    ...
    DELETE IFRS_OJK_REPORT_COVID WHERE DOWNLOAD_DATE = V_CURRDATE;

    /* Formatted on 8/28/2019 5:57:44 PM (QP5 v5.294) */
    /* Formatted on 8/28/2019 6:00:54 PM (QP5 v5.294) */
    INSERT INTO IFRS_OJK_REPORT_COVID (DOWNLOAD_DATE,
                             DATA_SOURCE,
                             IAS_CLASS,
                             IFRS9_CLASS,
                             CR_STAGE,
    ...
    FROM (
     SELECT A.DOWNLOAD_DATE,
       ...
       FROM IFRS_MASTER_ACCOUNT A
            ...
            LEFT JOIN (SELECT REPORT_DATE, MASTERID, STAGE CR_STAGE FROM IFRS_NOMINATIVE) D ON A.MASTERID = D.MASTERID AND A.DOWNLOAD_DATE = D.REPORT_DATE
   ...
   ORDER BY X.DOWNLOAD_DATE,
    ...
    INSERT INTO IFRS_OJK_REPORT_COVID
    (
    DOWNLOAD_DATE,
    DATa_SOURCE,
    IAS_CLASS,
    ...
    DELETE IFRS_OJK_REPORT_COVID WHERE RESERVED_VARCHAR_4 = 'T/DVALAS' AND DOWNLOAD_DATE = V_CURRDATE;COMMIT;
    DELETE IFRS_OJK_REPORT_COVID WHERE PRODUCT_CODE = 'DEPO-BI' AND DOWNLOAD_DATE = V_CURRDATE;COMMIT;


    COMMIT;

    /*QUERY INI HARUS DI RUNNING 2X*/
    UPDATE IFRS_OJK_REPORT_COVID
    SET ASET_KEUANGAN =
        CASE
