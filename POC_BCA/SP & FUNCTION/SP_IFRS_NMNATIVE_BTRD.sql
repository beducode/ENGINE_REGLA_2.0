CREATE OR REPLACE PROCEDURE SP_IFRS_NMNATIVE_BTRD(v_DOWNLOADDATECUR DATE DEFAULT ('1-JAN-1900')) AS
  V_CURRDATE DATE;
BEGIN
  ...
  loop
    ...
    dbms_output.put_line(REC.REPORT_DATE || '|' || REC.SOURCE_TYPE || '|' ||
                         ...
                         REC.BS_FLAG || '|' || REC.BI_CODE || '|' ||
                         REC.GOL_DEB || '|' || REC.PRODUCT_GROUP || '|' ||
                         REC.PRODUCT_TYPE || '|' || REC.PRODUCT_CODE || '|' ||
                         REC.PRODUCT_DESC || '|' || REC.START_DATE || '|' ||
                         REC.MATURITY_DATE || '|' || REC.RATING_CODE || '|' ||
