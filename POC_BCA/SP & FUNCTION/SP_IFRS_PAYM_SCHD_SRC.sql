...
BEGIN
    ...
    UPDATE /*+ PARALLEL(12) */ IFRS_PAYM_SCHD_ALL
    SET STATUS = 'A'
      ,END_DATE = NULL
    WHERE END_DATE >= V_CURRDATE;
    ...
    INSERT /*+ PARALLEL(12) */ INTO TMP1 (MASTERID)
    ...
    /*******   INSERT INTO HISTORY  *******/
    INSERT /*+ PARALLEL(12) */ INTO IFRS_PAYM_SCHD_ALL (
       DOWNLOAD_DATE
      ,MASTERID
      ,PMTDATE
      ,INTEREST_RATE
    ...
    SELECT /*+ PARALLEL(12) */ A.DOWNLOAD_DATE
        ...
        ,A.PRINCIPAL
        ,A.INTEREST
        ,0.000000 DISB_PERCENTAGE
        ,0.000000 DISB_AMOUNT
        ,B.PLAFOND PLAFOND
        ,CASE
            WHEN B.INTEREST_CALCULATION_CODE = 6
                THEN CASE
                        WHEN V_CALC_IDAYS = 0
                            THEN CASE
                                    WHEN (
                                            ROW_NUMBER() OVER (PARTITION BY A.MASTERID ORDER BY PMTDATE) - 1) = 1
                                        THEN CASE
                                                WHEN MONTHS_BETWEEN(A.PMTDATE, NVL(LAG(A.PMTDATE) OVER (PARTITION BY A.DOWNLOAD_DATE,A.MASTERID ORDER BY A.PMTDATE), A.PMTDATE)) = 0
                                                THEN 30
                                                ELSE MONTHS_BETWEEN(A.PMTDATE, NVL(LAG(A.PMTDATE) OVER (PARTITION BY A.DOWNLOAD_DATE,A.MASTERID ORDER BY A.PMTDATE), A.PMTDATE)) * 30
                                                END
                                    ELSE MONTHS_BETWEEN(A.PMTDATE, NVL(LAG(A.PMTDATE) OVER (PARTITION BY A.DOWNLOAD_DATE,A.MASTERID ORDER BY A.PMTDATE), A.PMTDATE)) * 30
                                    END
                        WHEN V_CALC_IDAYS = 1
                            THEN FN_CNT_DAYS_30_360(NVL(LAG(A.PMTDATE) OVER (PARTITION BY A.DOWNLOAD_DATE,A.MASTERID ORDER BY A.PMTDATE), A.PMTDATE), A.PMTDATE)
                        ELSE FN_CNT_DAYS_30_360(NVL(LAG(A.PMTDATE) OVER (PARTITION BY A.DOWNLOAD_DATE,A.MASTERID ORDER BY A.PMTDATE), A.PMTDATE), A.PMTDATE)
                        END
            ELSE NVL(A.PMTDATE - NVL(LAG(A.PMTDATE) OVER (PARTITION BY A.DOWNLOAD_DATE,A.MASTERID ORDER BY A.PMTDATE), A.PMTDATE),0)
            END I_DAYS
        ,(ROW_NUMBER() OVER (PARTITION BY A.DOWNLOAD_DATE,A.MASTERID ORDER BY PMTDATE)) COUNTER
        ,B.INTEREST_CALCULATION_CODE ICC
    ...
    INNER JOIN IFRS_IMA_AMORT_CURR B
    ON A.MASTERID = B.MASTERID
    LEFT JOIN IFRS_PAYM_SCHD C
    ON A.DOWNLOAD_DATE = C.DOWNLOAD_DATE AND A.MASTERID = C.MASTERID
    WHERE C.MASTERID IS NULL AND B.ECF_STATUS = 'Y';
...
--    COMMIT;
    ...
    INSERT /*+ PARALLEL(12) */ INTO TMP_T3
    SELECT /*+ PARALLEL(12) */ MASTERID FROM IFRS_IMA_AMORT_CURR
    ...
    INSERT /*+ PARALLEL(12) */ INTO TMP_IFRS_PAYM_SCHD
    (
       DOWNLOAD_DATE
      ,MASTERID
      ,PMTDATE
    ...
    JOIN TMP4 B
    ON  A.MASTERID = B.MASTERID
    AND A.PMTDATE <= B.EXPECTED_DATE;
    COMMIT;

    EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_PAYM_CORE_SRC';

    INSERT /*+ PARALLEL(12) */ INTO IFRS_PAYM_CORE_SRC (
       MASTERID
      ,PREV_PMT_DATE
      ,PMT_DATE
      ,INTEREST_RATE
    ...
    SELECT /*+ PARALLEL(12) */
      ...
      ,SCH.ICC
      ,SCH.GRACE_DATE
    FROM TMP_IFRS_PAYM_SCHD SCH
    WHERE SCH.MASTERID NOT IN (SELECT MASTERID FROM TMP_T3);
