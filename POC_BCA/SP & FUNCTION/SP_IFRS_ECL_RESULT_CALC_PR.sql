CREATE OR REPLACE PROCEDURE SP_IFRS_ECL_RESULT_CALC_PR(v_ECLID NUMBER DEFAULT(0), v_DOWNLOADDATE DATE DEFAULT('1-JAN-1900'))
AS
...
BEGIN
    ...
    SP_IFRS_INSRT_GTMP_FROM_IMA_PR(v_DOWNLOADDATE);

    EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_ECL_RESULT_DETAIL_CALC_PR';
    EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_ECL_RESULT_DETAIL_PR';
    ...
    MERGE INTO TMP_SBLC_OVERRIDE A
    USING IFRS_BUCKET_DETAIL B
    ON (A.CURRENCY = 'IDR' AND B.BUCKET_GROUP = 'PEFFI22_1' AND A.RATING = B.IMPAIRMENT_BUCKET)
    WHEN MATCHED THEN
    UPDATE SET
    ...
    MERGE INTO TMP_SBLC_OVERRIDE A
    USING IFRS_BUCKET_DETAIL B
    ON (A.CURRENCY != 'IDR' AND B.BUCKET_GROUP = 'SNPFI22_1' AND A.RATING = B.IMPAIRMENT_BUCKET)
    WHEN MATCHED THEN
    UPDATE SET
    ...
    ******************************************************/
    INSERT INTO IFRS_ECL_RESULT_DETAIL_CALC_PR (
        DOWNLOAD_DATE,
        ECL_MODEL_ID,
        MASTERID,
        BUCKET_GROUP,
...
--          AND A.ACCOUNT_STATUS = 'A'
    ...
    ******************************************************/
    INSERT INTO IFRS_ECL_RESULT_DETAIL_PR(
        DOWNLOAD_DATE,
        ECL_MODEL_ID,
        MASTERID,
        PF_SEGMENT_ID,
