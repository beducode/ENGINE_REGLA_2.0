CREATE OR REPLACE PROCEDURE SP_IFRS_ECL_RESULT_CALC (
...
BEGIN
    DELETE /*+ PARALLEL(IFRS_ECL_RESULT_DETAIL_CALC,8) */
           FROM IFRS_ECL_RESULT_DETAIL_CALC
          WHERE ECL_MODEL_ID = v_ECLID AND DOWNLOAD_DATE = v_DOWNLOADDATE;

    COMMIT;

    DELETE /*+ PARALLEL(IFRS_ECL_RESULT_DETAIL,8) */
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_SBLC_OVERRIDE';

    INSERT                             /* PARALLEL(TMP_PD_TERM_STRUCTURE,8) */
           INTO TMP_PD_TERM_STRUCTURE
        SELECT TS.EFF_DATE,
               TS.BASE_DATE,
               TS.PD_RULE_ID,
               TS.MODEL_ID,
         ...
         WHERE     TM_TYPE = 'YEAR'
               AND EXISTS
                       (SELECT 1
                          FROM IFRS_ECL_MODEL_CONFIG CF
                         WHERE     TS.PD_RULE_ID = CF.PD_MODEL_ID
                               AND CF.ECL_MODEL_ID = v_ECLID
                               AND CF.DOWNLOAD_DATE = v_DOWNLOADDATE
                               ...
                               AND TS.MODEL_ID =
                                   CASE
                                       ...
                                       ELSE
                                           0
                                   END --MODEL_ID '0' = TTC PD = NO FORWARD LOOKING
                                      )
        UNION ALL
    ...
    INSERT                               /* PARALLEL(TMP_IFRS_EAD_RESULT,8) */
           INTO TMP_IFRS_EAD_RESULT (DOWNLOAD_DATE,
                                     MASTERID,
                                     CURRENCY,
                                     EXCHANGE_RATE,
                                     ECL_MODEL_ID,
                                     ...
                                     MAX_COUNTER,
                                     SOURCE)
        SELECT                               /* PARALLEL(IFRS_EAD_RESULT,8) */
               DOWNLOAD_DATE,
               MASTERID,
               CURRENCY,
               EXCHANGE_RATE,
    ...
    INSERT                                 /* PARALLEL(TMP_SBLC_OVERRIDE,8) */
           INTO TMP_SBLC_OVERRIDE (ACCOUNT_NUMBER,
                                   CURRENCY,
                                   RATING,
                                   BUCKET_GROUP,
                                   BUCKET_ID,
        ...
        SELECT A.ACCOUNT_NUMBER,
               ...
               C.LGD_MODEL_ID
          FROM                                             /* PARALLEL(A,8) */
               GTMP_IFRS_MASTER_ACCOUNT  A
               JOIN                                        /* PARALLEL(B,8) */
                    TBLU_SBLC_OVERRIDE B
                   ON     A.ACCOUNT_NUMBER = B.ACCOUNT_NUMBER
                      AND B.DOWNLOAD_DATE = v_DOWNLOADDATE
               JOIN                                        /* PARALLEL(C,8) */
                    IFRS_ECL_MODEL_CONFIG C
                   ON     C.DOWNLOAD_DATE = B.DOWNLOAD_DATE
                      AND C.PF_SEGMENT_ID = 436
                      AND C.ECL_MODEL_ID = V_ECLID;

    COMMIT;

    MERGE INTO TMP_SBLC_OVERRIDE A
         USING IFRS_BUCKET_DETAIL B
    ...
    ******************************************************/
    INSERT /*+ PARALLEL(IFRS_ECL_RESULT_DETAIL_CALC,8) */
           INTO IFRS_ECL_RESULT_DETAIL_CALC (DOWNLOAD_DATE,
                                             ECL_MODEL_ID,
                                             MASTERID,
                                             BUCKET_GROUP,
                                             BUCKET_ID,
        ...
        SELECT /*+ PARALLEL(8) */
          ...
          FROM (select * from IFRS_MASTER_ACCOUNT where DOWNLOAD_DATE=v_DOWNLOADDATE)  A
         ...
         WHERE                          --A.DOWNLOAD_DATE = v_DOWNLOADDATE AND
               ...
               AND NVL (A.IFRS9_CLASS, ' ') != 'FVTPL'--          AND A.ACCOUNT_STATUS = 'A'
                                                      ;
    ...
    ******************************************************/
    INSERT /*+ PARALLEL(IFRS_ECL_RESULT_DETAIL,8) */
           INTO IFRS_ECL_RESULT_DETAIL (DOWNLOAD_DATE,
                                        ECL_MODEL_ID,
                                        MASTERID,
                                        PF_SEGMENT_ID,
                                        GROUP_SEGMENT,
