...
BEGIN
    ...
    VALUES( V_CURRDATE,SYSTIMESTAMP,'START','SP_IFRS_ACCT_SL_UPD_ACRU','');
    ...
    FROM IFRS_ACCT_SL_ACCRU_PREV
    WHERE STATUS='ACT'
    GROUP BY MASTERID, FLAG_CF;

    COMMIT;

    MERGE INTO IFRS_ACCT_SL_ACF A
    ...
    WHEN MATCHED THEN
    UPDATE
    SET A.N_ACCRU_PREV_COST=B.AMOUNT;
    ...
    WHEN MATCHED THEN
    UPDATE
    SET  A.N_ACCRU_PREV_FEE=B.AMOUNT;

    COMMIT;


    INSERT INTO IFRS_AMORT_LOG(DOWNLOAD_DATE,DTM,OPS,PROCNAME,REMARK)
    VALUES(V_CURRDATE,SYSTIMESTAMP,'END','SP_IFRS_ACCT_SL_UPD_ACRU','');
