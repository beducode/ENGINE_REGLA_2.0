...
BEGIN
    ...
    SELECT  CURRDATE ,PREVDATE, LAST_DAY(CURRDATE)
    INTO V_CURRDATE, V_PREVDATE, V_ENDOFMONTH
    FROM  IFRS_PRC_DATE_AMORT ;

	IF V_ENDOFMONTH = V_CURRDATE THEN
        EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_PAYM_SCHD_MTM';

        INSERT /*+ PARALLEL(12) */ INTO IFRS_PAYM_SCHD_MTM
        (
          MASTERID
          ,PMTDATE
          ,INTEREST_RATE
        ...
        MERGE INTO IFRS_PAYM_SCHD_MTM X
        USING IFRS_IMA_AMORT_CURR Z
        ON (X.MASTERID = Z.MASTERID AND Z.DOWNLOAD_DATE = V_CURRDATE)
        WHEN MATCHED THEN
        UPDATE SET X.PV_CF = (X.PRINCIPAL + X.INTEREST) / POWER((1 + Z.MARKET_RATE / 12 / 100), X.COUNTER);

        COMMIT;

        DELETE /*+ PARALLEL(12) */ IFRS_EIR_ADJUSTMENT
        WHERE DOWNLOAD_DATE = V_CURRDATE;

        COMMIT;

        INSERT /*+ PARALLEL(12) */ INTO IFRS_EIR_ADJUSTMENT (
            DOWNLOAD_DATE
            ,MASTERID
            ,ACCOUNT_NUMBER
            ,IFRS9_CLASS
...
--      VALUES  ( V_CURRDATE ,0 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,0 ,'JUST STARTED') ;
--
--      ----COMMIT;
--      --IF @V_CURRDATE < @CUT_OFF_DATE RETURN
--
--      EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_IFRS_SCHEDULE_MAIN_MTM' ;
--      EXECUTE IMMEDIATE 'TRUNCATE TABLE IFRS_PAYM_SCHD_MTM' ;
--
--      INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--      VALUES  ( V_CURRDATE ,1 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,1 ,'INSERT TMP_IFRS_SCHEDULE_MAIN_MTM') ;
--
--
--      ----COMMIT;
--
--      INSERT  INTO TMP_IFRS_SCHEDULE_MAIN_MTM
--      ( DOWNLOAD_DATE ,
--        MASTERID ,
--        ACCOUNT_NUMBER ,
--        BRANCH_CODE ,
...
--
--      INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--      VALUES  ( V_CURRDATE ,1 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,3 ,'INITIAL PROCESS') ;
--
--
--      ----COMMIT;
--
...
--
--
--      EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_SCHEDULE_CURR_MTM_TEMP' ;
--      EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_SCHEDULE_PREV_MTM_TEMP' ;
--      EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_SCHEDULE_CURR' ;
--      EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_SCHEDULE_PREV' ;
--
--      INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--      VALUES  ( V_CURRDATE ,1 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,4 ,'INSERT TMP_SCHEDULE_CURR') ;
--      ----COMMIT;
--      COMMIT;
--
...
--      FROM TMP_IFRS_SCHEDULE_MAIN_MTM A
...
--      COMMIT;
--
--      INSERT  INTO TMP_SCHEDULE_CURR_MTM_TEMP
--      SELECT  *
...
--      COMMIT;
--      INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--      VALUES  ( V_CURRDATE ,1 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,5 ,'INSERT IFRS_PAYM_SCHD') ;
--
--
--      COMMIT;
...
--      FROM    TMP_IFRS_SCHEDULE_MAIN_MTM ;
--
...
--        --- START ADD YAHYA--
--
--				DELETE FROM TMP_MIN_MAX_DATE;
--
--				INSERT INTO TMP_MIN_MAX_DATE (ACCOUNT_NUMBER,MIN_DATE)
--				SELECT A.ACCOUNT_NUMBER,MIN(A.DATE_START) AS MIN_DATE -- INTO #TT_MIN_MAX_DATE
...
--
--        INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--        VALUES  ( V_CURRDATE ,2 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,V_COUNTER_PAY ,'PAYMENT SCHEDULE LOOPING') ;
--
--
--        COMMIT;
--        EXECUTE IMMEDIATE 'TRUNCATE TABLE TMP_SCHEDULE_PREV' ;
...
--
--
--        INSERT  INTO TMP_SCHEDULE_PREV_MTM_TEMP
...
--
--      INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--      VALUES  ( V_CURRDATE ,3 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,1 ,'PAYMENT SCHEDULE EXCEPTIONS') ;
--
--      COMMIT;
...
--
--      INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--      VALUES  ( V_CURRDATE ,3 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,2 ,'INSERT IFRS_EXCEPTION_DETAILS') ;
--
--
--      COMMIT;
...
--      MERGE INTO IFRS_PAYM_SCHD_MTM X
--      USING IFRS_IMA_AMORT_CURR Z ON (X.MASTERID = Z.MASTERID AND Z.DOWNLOAD_DATE = V_CURRDATE)
--      WHEN MATCHED THEN
--      UPDATE SET X.PV_CF= (X.PRINCIPAL + X.INTEREST) / POWER((1 + Z.MARKET_RATE / 12 / 100), X.COUNTER) ;
--
--      COMMIT;
--
--      DELETE FROM IFRS_EIR_ADJUSTMENT
--      WHERE DOWNLOAD_DATE = V_CURRDATE;
--      COMMIT;
--
...
--                         SUM((X.PRINCIPAL + X.INTEREST) / POWER((1 + Z.MARKET_RATE / 12 / 100), X.COUNTER)) AS TOTAL_PV_CF
--                  FROM IFRS_PAYM_SCHD_MTM X
--								  INNER JOIN IFRS_IMA_AMORT_CURR Z
--									 ON X.MASTERID = Z.MASTERID
--								  WHERE Z.DOWNLOAD_DATE = V_CURRDATE
--								  GROUP BY X.MASTERID) B
--        ON A.MASTERID = B.MASTERID
--      WHERE A.DOWNLOAD_DATE = V_CURRDATE
...
--
--      INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--      VALUES  ( V_CURRDATE ,4 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,1,'MARKET TO MARKET') ;
--
--      INSERT  INTO IFRS_BATCH_LOG_DETAILS( DOWNLOAD_DATE ,BATCH_ID ,BATCH_ID_HEADER ,BATCH_NAME ,PROCESS_NAME ,START_DATE ,CREATEDBY ,COUNTER ,REMARKS)
--      VALUES  ( V_CURRDATE ,99 ,V_LOG_ID ,'PMTSCHD' ,'SP_IFRS_PAYMENT_SCHEDULE_MTM' ,SYSTIMESTAMP ,'IFRS ENGINE' ,99,'JUST ENDED') ;
--
--      COMMIT; --=========
--    END IF;
END ;