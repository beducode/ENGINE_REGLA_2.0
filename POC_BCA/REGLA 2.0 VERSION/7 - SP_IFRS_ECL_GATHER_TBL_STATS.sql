CREATE OR REPLACE PROCEDURE IFRS9_BCA.SP_IFRS_ECL_GATHER_TBL_STATS_BCA (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_ECL_GATHER_TBL_STATS_BCA';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
    ...
    IF V_PRC = 'S' THEN
        ...
        V_STR_QUERY := 'CREATE TABLE ' || V_OWNER || '.' || V_TABLEINSERT1 ||
                       ' AS SELECT * FROM ' || V_OWNER || '.IFRS_ECL_RESULT_HEADER WHERE DOWNLOAD_DATE = TO_DATE(''' || TO_CHAR(V_CURRDATE,'YYYY-MM-DD') || ''',''YYYY-MM-DD'') AND 1=0';
        EXECUTE IMMEDIATE V_STR_QUERY;
    END IF;
    COMMIT;
    ...
    DBMS_STATS.UNLOCK_TABLE_STATS ( OWNNAME=>V_OWNER, TABNAME=>V_TABLEINSERT1);
    DBMS_STATS.UNLOCK_TABLE_STATS ( OWNNAME=>V_OWNER, TABNAME=>V_TABLESELECT1);
    DBMS_STATS.UNLOCK_TABLE_STATS ( OWNNAME=>V_OWNER, TABNAME=>V_TABLESELECT2);

    DBMS_STATS.GATHER_TABLE_STATS ( OWNNAME=>V_OWNER, TABNAME=>V_TABLEINSERT1,DEGREE=>8);
    DBMS_STATS.GATHER_TABLE_STATS ( OWNNAME=>V_OWNER, TABNAME=>V_TABLESELECT1,DEGREE=>8);
    DBMS_STATS.GATHER_TABLE_STATS ( OWNNAME=>V_OWNER, TABNAME=>V_TABLESELECT2,DEGREE=>8);
