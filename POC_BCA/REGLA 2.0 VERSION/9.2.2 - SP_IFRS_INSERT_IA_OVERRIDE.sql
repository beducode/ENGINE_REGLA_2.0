CREATE OR REPLACE PROCEDURE IFRS9_BCA.SP_IFRS_INSERT_IA_OVERRIDE (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_INSERT_IA_OVERRIDE';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
    ...
    IF V_PRC = 'S' THEN
        ...
        V_STR_QUERY := 'CREATE TABLE ' || V_OWNER || '.' || V_TABLEINSERT1 ||
                       ' AS SELECT * FROM ' || V_OWNER || '.IFRS_IA_OVERRIDED_HIST';
        EXECUTE IMMEDIATE V_STR_QUERY;
    END IF;
    COMMIT;
