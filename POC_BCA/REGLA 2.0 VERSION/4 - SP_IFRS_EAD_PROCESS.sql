...
BEGIN
    ...
    WHERE TABLE_NAME = 'IFRS_EAD_RESULT';

    IF V_COUNT > 0 THEN

        EXECUTE IMMEDIATE 
        'ALTER TABLE ' || V_OWNER || '.IFRS_EAD_RESULT TRUNCATE PARTITION DOWNLOAD_DATE_' || TO_CHAR(V_CURRDATE,'YYYY_MM');

    ELSE

        EXECUTE IMMEDIATE 
        'DELETE FROM ' || V_OWNER || '.IFRS_EAD_RESULT WHERE DOWNLOAD_DATE = TO_DATE(''' || TO_CHAR(V_CURRDATE,'YYYY-MM-DD') || ''',''YYYY-MM-DD'')';
   ...
   (
    ...
    JOIN ' || V_OWNER || '.IFRS_ECL_MODEL_CONFIG C ON C.PF_SEGMENT_ID = A.SEGMENT_RULE_ID AND A.DOWNLOAD_DATE = C.DOWNLOAD_DATE
    ...
    FROM ' || V_OWNER || '.TMP_IFRS_IMA_EAD A
    INNER JOIN ' || V_OWNER || '.IFRS_ECL_MODEL_CONFIG F ON A.DOWNLOAD_DATE = F.DOWNLOAD_DATE AND A.ECL_MODEL_ID = F.ECL_MODEL_ID AND A.SEGMENT_RULE_ID = F.PF_SEGMENT_ID AND A.EAD_RULE_ID = F.EAD_MODEL_ID
    ...
    LEFT JOIN (
        ...
        (
            ...
            JOIN ' || V_OWNER || '.IFRS_ECL_MODEL_CONFIG B
   ...
   V_STR_QUERY := 'MERGE INTO ' || V_OWNER || '.TMP_IFRS_IMA_EAD X
    ...
    V_STR_QUERY := 'INSERT INTO ' || V_OWNER || '.IFRS_EAD_RESULT
    (
    PKID,
    DOWNLOAD_DATE,
    MASTERID,
    ...
    (
		...
		JOIN ' || V_OWNER || '.IFRS_ECL_MODEL_CONFIG C2
