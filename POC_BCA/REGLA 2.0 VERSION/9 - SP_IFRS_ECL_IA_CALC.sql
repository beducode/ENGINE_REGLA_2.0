CREATE OR REPLACE PROCEDURE IFRS9_BCA.SP_IFRS_ECL_IA_CALC (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_ECL_IA_CALC';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
    ...
    ----------------------------------------------------------------
    V_STR_QUERY := 'BEGIN IFRS9_BCA.SP_IFRS_ECL_IA_INITIAL_RUN(:1, :2, :3, :4); END;';

    EXECUTE IMMEDIATE V_STR_QUERY
    USING V_RUNID, V_CURRDATE, V_SYSCODE, V_PRC;
    COMMIT;

    V_STR_QUERY := 'BEGIN IFRS9_BCA.SP_IFRS_IA_SEQUENCE(:1, :2, :3, :4, :5); END;';

    EXECUTE IMMEDIATE V_STR_QUERY
    USING V_RUNID, V_CURRDATE, V_SYSCODE, V_PRC, V_UPLOADID;
    COMMIT;

    V_STR_QUERY := 'BEGIN IFRS9_BCA.SP_IFRS_ECL_IA_TO_CA(:1, :2, :3, :4); END;';

    EXECUTE IMMEDIATE V_STR_QUERY
    USING V_RUNID, V_CURRDATE, V_SYSCODE, V_PRC;
    COMMIT;

    V_STR_QUERY := 'BEGIN IFRS9_BCA.SP_IFRS_ECL_UNPROCESSED_IA(:1, :2, :3, :4); END;';
