CREATE OR REPLACE PROCEDURE IFRS9_BCA.SP_IFRS_PD_MIG_LOGIT_ODR_DEV (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_PD_MIG_LOGIT_ODR_DEV';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
    ...
    IF V_PRC = 'S' THEN
        ...
        V_STR_QUERY := 'CREATE TABLE ' || V_OWNER || '.' || V_TABLEINSERT1 ||
                       ' AS SELECT * FROM ' || V_OWNER || '.IFRS_PD_MIG_LOGIT_ODR';
        EXECUTE IMMEDIATE V_STR_QUERY;
    END IF;
    COMMIT;
    ...
    (
        ...
        WHERE EFF_DATE = ADD_MONTHS(TO_DATE(''' || TO_CHAR(V_CURRDATE,'YYYY-MM-DD') || ''',''YYYY-MM-DD''), -12)
    ) B
    ON (A.EFF_DATE = TO_DATE(''' || TO_CHAR(V_CURRDATE,'YYYY-MM-DD') || ''',''YYYY-MM-DD'') AND A.PD_RULE_ID = B.PD_RULE_ID)
    WHEN MATCHED THEN
    UPDATE SET
