CREATE OR REPLACE PROCEDURE IFRS9_BCA.SP_IFRS_PD_VAS_CORRELATION_DEV (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_PD_VAS_CORRELATION_DEV';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
    ...
    IF V_PRC = 'S' THEN
        ...
        V_STR_QUERY := 'CREATE TABLE ' || V_OWNER || '.' || V_TABLEINSERT1 ||
                       ' AS SELECT * FROM ' || V_OWNER || '.IFRS_PD_VAS_CORRELATION';
        EXECUTE IMMEDIATE V_STR_QUERY;
    END IF;
    COMMIT;
