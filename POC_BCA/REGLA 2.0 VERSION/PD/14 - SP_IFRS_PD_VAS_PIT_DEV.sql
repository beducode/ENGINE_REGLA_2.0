CREATE OR REPLACE PROCEDURE IFRS9_BCA.SP_IFRS_PD_VAS_PIT_DEV (
...
AS
    ...
    -- VARIABLES
    ----------------------------------------------------------------
    V_SP_NAME     VARCHAR2(100) := 'SP_IFRS_PD_VAS_PIT_DEV';
    V_OWNER       VARCHAR2(30);
    V_CURRDATE      DATE;
...
BEGIN
    ...
    ----------------------------------------------------------------
    IF V_PRC = 'S' THEN 
        V_TABLEINSERT1 := 'IFRS_PD_VAS_PIT_' || V_RUNID;
        V_TABLESELECT1 := 'TMP_IFRS_PD_RUNNING_DATE_' || V_RUNID;
        V_TABLESELECT2 := 'IFRS_PD_VAS_CORRELATION_' || V_RUNID;
        V_TABLEPDCONFIG := 'IFRS_PD_RULES_CONFIG_' || V_RUNID;
    ELSE 
        V_TABLEINSERT1 := 'IFRS_PD_VAS_PIT';
        V_TABLESELECT1 := 'TMP_IFRS_PD_RUNNING_DATE';
        V_TABLESELECT2 := 'IFRS_PD_VAS_CORRELATION';
    ...
    IF V_PRC = 'S' THEN
        ...
        V_STR_QUERY := 'CREATE TABLE ' || V_OWNER || '.' || V_TABLEINSERT1 ||
                       ' AS SELECT * FROM ' || V_OWNER || '.IFRS_PD_VAS_PIT';
        EXECUTE IMMEDIATE V_STR_QUERY;
    END IF;
    COMMIT;
    ...
    V_STR_QUERY := 'DELETE ' || V_OWNER || '.IFRS_PD_VAS_PIT
    ...
    V_STR_QUERY := 'INSERT INTO ' || V_OWNER || '.IFRS_PD_VAS_PIT
	(
		EFF_DATE,
		PD_RULE_ID,
		BUCKET_GROUP,
	...
	JOIN ' || V_OWNER || '.IFRS_PD_VAS_ME_PROJECTION C
