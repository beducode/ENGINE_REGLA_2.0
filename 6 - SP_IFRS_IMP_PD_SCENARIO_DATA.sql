...
BEGIN 
    ...
    FROM IFRS_PD_RULES_CONFIG A INNER JOIN  IFRS_SCENARIO_SEGMENT_GENERATE_QUERY B ON A.SEGMENTATION_ID= B.RULE_ID   AND B.SEGMENT_TYPE = ''PD_SEGMENT''    
    ...
    LOOP
        ...
        JOIN IFRS_PD_RULES_CONFIG B ON B.PKID = ' || V_PD_RULE_ID || '         
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE DEFAULT_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE MAX_BUCKET_' || P_RUNID || ' AS 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE FP_ACCOUNT_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE PD_DATA_12M_' || P_RUNID || ' AS
    ...
    FROM PD_DATA_12M_' || P_RUNID || ' AS B, IFRS_PD_RULES_CONFIG AS C 
