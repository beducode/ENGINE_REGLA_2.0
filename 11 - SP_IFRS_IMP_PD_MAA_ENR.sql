...
BEGIN 
    ...
    INNER JOIN IFRS_PD_RULES_CONFIG B ON
    A.PD_RULE_ID = B.PKID
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TOTAL_COUNT_' || P_RUNID || ' AS 
    ...
    SET PERCENTAGE = CASE WHEN B.TOTAL_ACCOUNT = 0 THEN NULL ELSE CAST(A.CALC_AMOUNT AS FLOAT)/CAST(B.TOTAL_ACCOUNT AS FLOAT) END,
    TOTAL_ACCOUNT = B.TOTAL_ACCOUNT
    FROM TOTAL_COUNT_' || P_RUNID || ' AS B, IFRS_PD_RULES_CONFIG AS C
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ENR_CURR_' || P_RUNID || ' AS 
    ...
    LOOP
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE NOT_FOUND_BUCKET_' || P_RUNID || ' AS 
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ENR_MAA_BASE_' || P_RUNID || ' AS 
        SELECT DOWNLOAD_DATE
        ,BASE_DATE
        ,TO_DATE
        ,PD_RULE_ID
        ...
        INNER JOIN IFRS_PD_RULES_CONFIG C ON A.PD_RULE_ID = C.PKID
