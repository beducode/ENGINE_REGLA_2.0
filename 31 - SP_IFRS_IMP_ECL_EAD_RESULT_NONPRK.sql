---- DROP PROCEDURE SP_IFRS_IMP_ECL_EAD_RESULT_NONPRK;

CREATE OR REPLACE PROCEDURE SP_IFRS_IMP_ECL_EAD_RESULT_NONPRK(
...
BEGIN 
    ...
    IF P_PRC = 'S' THEN
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'DROP TABLE IF EXISTS TEMP_MAX_VALUE';
        EXECUTE (V_STR_QUERY);

        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TEMP_MAX_VALUE AS SELECT MAX(LIFETIME - 1) AS LIFETIME FROM ' || V_TABLEINSERT1 || '';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT3 || ' AS SELECT * FROM IFRS_EAD_RESULT_NONPRK WHERE 0=1';
        EXECUTE (V_STR_QUERY);
    END IF;

    SELECT LIFETIME INTO V_MAX_LIFETIME FROM TEMP_MAX_VALUE;
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_LISTDATE_' || P_RUNID || ' AS WITH RECURSIVE CTE_DATE AS (
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_MAXDATE_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_IFRS_ECL_MODEL_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE SCHD_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE IFRS_EAD_PAYM_NONPRK_' || P_RUNID || ' AS WITH CTE_PAYM_SCHD ( DOWNLOAD_DATE, MASTERID, PMTDATE, OSPRN, PRINCIPAL, END_DATE, RN ) AS (
    SELECT
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_CTE_EAD_' || P_RUNID || ' AS WITH CTE_EAD AS (
    SELECT
        DOWNLOAD_DATE,
        MASTERID,
        GROUP_SEGMENT,
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE ZEROING_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE UPDATE_TMP_CTE_EAD_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE MINUS_LIFETIME_' || P_RUNID || ' AS
    SELECT MASTERID
    FROM ' || V_TABLEINSERT3 || '
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DROP TABLE IF EXISTS TMP_IFRS_EAD_RESULT_NONPRK_' || P_RUNID || '';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_IFRS_EAD_RESULT_NONPRK_' || P_RUNID || ' AS
    SELECT * FROM ' || V_TABLEINSERT3 || ' WHERE 1 = 2';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DROP TABLE IF EXISTS MINUS_LIFETIME';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE MINUS_LIFETIME AS
    SELECT * FROM MINUS_LIFETIME_' || P_RUNID || '';
    EXECUTE (V_STR_QUERY);
    ...
    IF((SELECT COUNT(1) FROM MINUS_LIFETIME) > 0) THEN
        ...
        WHILE V_START <= V_END LOOP
            EXECUTE 'INSERT INTO TMP_IFRS_EAD_RESULT_NONPRK_' || P_RUNID || ' (
                DOWNLOAD_DATE,
                MASTERID,
                GROUP_SEGMENT,
                SEGMENT,
        ...
        SELECT
            ...
            COLL_AMOUNT,
            SEGMENT_FLAG
        FROM TMP_IFRS_EAD_RESULT_NONPRK_' || P_RUNID || '';
        EXECUTE (V_STR_QUERY);
    ...
    RAISE NOTICE 'SP_IFRS_IMP_ECL_EAD_RESULT_NONPRK | AFFECTED RECORD : %', V_RETURNROWS2;
    -------- ====== BODY ======

    -------- ====== LOG ======
    V_TABLEDEST = V_TABLEINSERT3;
    V_COLUMNDEST = '-';
    V_SPNAME = 'SP_IFRS_IMP_ECL_EAD_RESULT_NONPRK';
    V_OPERATION = 'INSERT';
