...
BEGIN 
    ...
    FOR V_RETENTION_ID, V_CONNECTION_ID, V_TABLESOURCE, V_TABLEDEST, V_SEQUENCE_PARAM, V_TABLE_CONDITION, V_CONNECTION_KEY IN
    EXECUTE 'SELECT * FROM dblink(''link_ntt_parameter'', ''SELECT retention_id, connection_id, table_source, table_destination, sequence, table_condition, connection_key FROM VW_DATA_RETENTION_POLICY'') 
    AS IFRS_RETENTION_MASTER_GET (RETENTION_ID VARCHAR(50) ,CONNECTION_ID BIGINT, TABLE_SOURCE VARCHAR(100), TABLE_DESTINATION VARCHAR(100), SEQUENCE SMALLINT, TABLE_CONDITION TEXT, CONNECTION_KEY TEXT);'
