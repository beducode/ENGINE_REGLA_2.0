...
BEGIN 
    ...
    FOR V_CLEANTABLENAME IN
        ...
        AND TABLE_SCHEMA = 'public' 
        AND TABLE_TYPE = 'BASE TABLE'
        AND RIGHT(TABLE_NAME,LENGTH('S_000_0000')) = LOWER('S_000_0000')
	LOOP	
		V_STR_QUERY := '';
    ...
    --     AND RIGHT(LEFT(SEQUENCE_NAME,LENGTH(SEQUENCE_NAME)-9),4) NOT IN (SELECT LOWER(RIGHT(RUN_ID,5)) FROM (SELECT MAX(PROCESS_DATE), RUN_ID FROM IFRS_LOGS_PROCESS
    --     WHERE RUN_ID <> 'S0000'
    --     GROUP BY RUN_ID
    --     ORDER BY MAX(PROCESS_DATE) DESC
    --     LIMIT 5) LOGS)
