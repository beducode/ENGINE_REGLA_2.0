---- DROP PROCEDURE SP_FETCH_IFRS_MASTER_PAYMENT_SCHEDULE;

CREATE OR REPLACE PROCEDURE SP_FETCH_IFRS_MASTER_PAYMENT_SCHEDULE(
...
BEGIN 
    ...
    IF P_PRC = 'S' THEN 
        V_TABLEINSERT1 := 'IFRS_STG_PAYM_SCHD_' || P_RUNID || '';
    ELSE 
        V_TABLEINSERT1 := 'IFRS_STG_PAYM_SCHD';
    END IF;
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT1 || ' AS SELECT * FROM IFRS_STG_PAYM_SCHD WHERE 1=0 ';
        EXECUTE (V_STR_QUERY);
    END IF;
    -------- ====== PRE SIMULATION TABLE ======
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT1 || ' 
        (    
            ...
            END AS INTEREST 
        FROM DBLINK(''ifrs9_stg'', ''SELECT * FROM IMA_SCHD'') 
        AS A ( 
            DOWNLOAD_DATE DATE 
    ...
    RAISE NOTICE 'SP_FETCH_IFRS_MASTER_PAYMENT_SCHEDULE | AFFECTED RECORD : %', V_RETURNROWS2;
    ---------- ====== BODY ======

    -------- ====== LOG ======
    V_TABLEDEST = V_TABLEINSERT1;
    V_COLUMNDEST = '-';
    V_SPNAME = 'SP_FETCH_IFRS_MASTER_PAYMENT_SCHEDULE';
    V_OPERATION = 'INSERT';
