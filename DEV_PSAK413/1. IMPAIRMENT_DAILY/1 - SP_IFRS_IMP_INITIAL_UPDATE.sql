...
BEGIN 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS IDX_CUST_' || V_TABLENAME || ' ';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IDX_CUST_' || V_TABLENAME || ' ON ' || V_TMPTABLE4 || ' (CUSTOMER_NUMBER) ';
    EXECUTE (V_STR_QUERY);
