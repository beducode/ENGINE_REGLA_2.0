...
BEGIN 
    ...
    IF P_PRC = 'S' THEN 
        ...
        V_TABLEINSERT7 := 'IFRS_IMP_DEFAULT_STATUS_' || P_RUNID || '';
    ...
    ELSE 
        ...
        V_TABLEINSERT7 := 'IFRS_IMP_DEFAULT_STATUS';
    ...
    IF P_PRC = 'S' THEN
        ...
        (DOWNLOAD_DATE ASC NULLS LAST, MASTERID ASC NULLS LAST, PD_RULE_ID ASC NULLS LAST, PD_UNIQUE_ID ASC NULLS LAST)
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT6 || ' AS SELECT * FROM IFRS_PD_SCENARIO_DATA WHERE 0=1';
        EXECUTE (V_STR_QUERY);

        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_' || V_TABLEINSERT6 || ' ';
        EXECUTE (V_STR_QUERY);

        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IF NOT EXISTS NCI_' || V_TABLEINSERT6 || '
    ...
    FROM ' || V_TABLEPDCONFIG || ' A INNER JOIN  ' || V_TABLEINSERT5 || ' B ON A.SEGMENTATION_ID= B.RULE_ID   AND B.SEGMENT_TYPE = ''PD_SEGMENT''    
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT6 || ' SELECT * FROM ' || V_TABLEINSERT4 || '';
    EXECUTE (V_STR_QUERY);
    ...
    CALL SP_IFRS_IMP_PD_SCENARIO_DATA_NOLAG(P_RUNID, NULL, P_PRC);
