...
BEGIN 
    ...
    INNER JOIN IFRS_PD_TERM_STRUCTURE_NOFL C ON B.PD_MODEL_ID = C.PD_RULE_ID AND B.EFF_DATE = C.DOWNLOAD_DATE
    ...
    SELECT *, 
    CASE WHEN SUM(PD_SCALAR) OVER (PARTITION BY DOWNLOAD_DATE,EIL_MODEL_ID, PD_RULE_ID,BUCKET_ID,SEGMENTATION_ID ORDER BY FL_SEQ)  > 1 THEN CAST( 1 AS FLOAT) 
    ELSE CAST(SUM(PD_SCALAR) OVER (PARTITION BY DOWNLOAD_DATE,EIL_MODEL_ID, PD_RULE_ID,BUCKET_ID,SEGMENTATION_ID ORDER BY FL_SEQ) AS FLOAT) END  AS CUMULATIVE_PD_SCALAR2
    ...
    SELECT *,
    CAST(COALESCE(CUMULATIVE_PD_SCALAR2,0) AS FLOAT) - CAST(COALESCE(LAG(CUMULATIVE_PD_SCALAR2) OVER (PARTITION BY DOWNLOAD_DATE,EIL_MODEL_ID, PD_RULE_ID,BUCKET_ID,SEGMENTATION_ID ORDER BY FL_SEQ),0) AS FLOAT) AS PD_FINAL2
