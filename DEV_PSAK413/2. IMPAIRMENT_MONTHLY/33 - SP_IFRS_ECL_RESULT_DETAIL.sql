...
BEGIN 
    ...
    IF(P_PRC = 'S') THEN
      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_TMP_IFRS_ECL_MODEL_ECL_' || P_RUNID || ' ';
      EXECUTE (V_STR_QUERY);

      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IF NOT EXISTS NCI_TMP_IFRS_ECL_MODEL_ECL_' || P_RUNID || '
      ON TMP_IFRS_ECL_MODEL_ECL_' || P_RUNID || ' USING BTREE
      (EIL_MODEL_ID ASC NULLS LAST, EAD_MODEL_ID ASC NULLS LAST, SEGMENTATION_ID ASC NULLS LAST, LGD_MODEL_ID ASC NULLS LAST, PD_MODEL_ID ASC NULLS LAST)
    ...
    IF(P_PRC = 'S') THEN
      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_TMP_IFRS_ECL_PD_MONTHLY_ECL_' || P_RUNID || ' ';
      EXECUTE (V_STR_QUERY);

      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IF NOT EXISTS NCI_TMP_IFRS_ECL_PD_MONTHLY_ECL_' || P_RUNID || '
      ON TMP_IFRS_ECL_PD_MONTHLY_ECL_' || P_RUNID || ' USING BTREE
      (DOWNLOAD_DATE ASC NULLS LAST, PD_RULE_ID ASC NULLS LAST, PD_ME_MODEL_ID ASC NULLS LAST, BUCKET_ID ASC NULLS LAST)
      TABLESPACE PG_DEFAULT';
      EXECUTE (V_STR_QUERY);
    END IF;
    ...
    IF(P_PRC = 'S') THEN
      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_TMP_IFRS_ECL_PD_FL_MONTHLY_ECL_' || P_RUNID || ' ';
      EXECUTE (V_STR_QUERY);

      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IF NOT EXISTS NCI_TMP_IFRS_ECL_PD_FL_MONTHLY_ECL_' || P_RUNID || '
      ON TMP_IFRS_ECL_PD_FL_MONTHLY_ECL_' || P_RUNID || ' USING BTREE
      (DOWNLOAD_DATE ASC NULLS LAST, PD_RULE_ID ASC NULLS LAST, SEGMENTATION_ID ASC NULLS LAST, BUCKET_ID ASC NULLS LAST)
      TABLESPACE PG_DEFAULT';
      EXECUTE (V_STR_QUERY);
    END IF;
    ...
    IF(P_PRC = 'S') THEN
      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_TMP_IFRS_ECL_PD_YEARLY_' || P_RUNID || ' ';
      EXECUTE (V_STR_QUERY);

      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IF NOT EXISTS NCI_TMP_IFRS_ECL_PD_YEARLY_' || P_RUNID || '
      ON TMP_IFRS_ECL_PD_YEARLY_' || P_RUNID || ' USING BTREE
      (DOWNLOAD_DATE ASC NULLS LAST, PD_RULE_ID ASC NULLS LAST, FL_SEQ ASC NULLS LAST, BUCKET_ID ASC NULLS LAST)
      TABLESPACE PG_DEFAULT';
      EXECUTE (V_STR_QUERY);
    END IF;
    ...
    IF(P_PRC = 'S') THEN
      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_TMP_IFRS_ECL_PD_FL_YEARLY_' || P_RUNID || ' ';
      EXECUTE (V_STR_QUERY);

      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IF NOT EXISTS NCI_TMP_IFRS_ECL_PD_FL_YEARLY_' || P_RUNID || '
      ON TMP_IFRS_ECL_PD_FL_YEARLY_' || P_RUNID || ' USING BTREE
      (DOWNLOAD_DATE ASC NULLS LAST, PD_RULE_ID ASC NULLS LAST, SEGMENTATION_ID ASC NULLS LAST, BUCKET_ID ASC NULLS LAST)
      TABLESPACE PG_DEFAULT';
      EXECUTE (V_STR_QUERY);
    END IF;
    ...
    IF(P_PRC = 'S') THEN
      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_TMP_IFRS_ECL_LGD_' || P_RUNID || ' ';
      EXECUTE (V_STR_QUERY);

      V_STR_QUERY := '';
      V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IF NOT EXISTS NCI_TMP_IFRS_ECL_LGD_' || P_RUNID || '
      ON TMP_IFRS_ECL_LGD_' || P_RUNID || ' USING BTREE
      (DOWNLOAD_DATE ASC NULLS LAST, LGD_RULE_ID ASC NULLS LAST, FL_SEQ ASC NULLS LAST)
      TABLESPACE PG_DEFAULT';
      EXECUTE (V_STR_QUERY);
    END IF;
