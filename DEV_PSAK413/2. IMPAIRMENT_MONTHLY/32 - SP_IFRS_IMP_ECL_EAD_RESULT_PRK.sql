...
BEGIN 
    ...
    IF P_PRC = 'S' THEN 
        ...
        V_TABLEINSERT4 := 'IFRS_EAD_RESULT_PRK_MONTHLY_' || P_RUNID || '';
    ELSE 
        ...
        V_TABLEINSERT4 := 'IFRS_EAD_RESULT_PRK_MONTHLY';
    END IF;
    ...
    WHERE (              
    A.PRODUCT_TYPE IN (''HASANAH_CARD'')               
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT4 || ' AS SELECT          
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_' || V_TABLEINSERT4 || ' ';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IF NOT EXISTS NCI_' || V_TABLEINSERT4 || '
    ON ' || V_TABLEINSERT4 || ' USING BTREE
    (CCF_RULES_ID ASC NULLS LAST, EAD_MODEL_ID ASC NULLS LAST, R_NUMBER ASC NULLS LAST)
    TABLESPACE PG_DEFAULT';
    EXECUTE (V_STR_QUERY);
    -------- ====== BODY ======
