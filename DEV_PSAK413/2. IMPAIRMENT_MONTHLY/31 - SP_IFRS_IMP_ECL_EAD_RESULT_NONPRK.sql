...
BEGIN 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DROP TABLE IF EXISTS TMP_IFRS_EAD_PAYM_NONPRK_' || P_RUNID || '';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE TMP_IFRS_EAD_PAYM_NONPRK_' || P_RUNID || ' AS WITH CTE_PAYM_SCHD ( DOWNLOAD_DATE, MASTERID, PMTDATE, OSPRN, PRINCIPAL, END_DATE, RN ) AS (
    SELECT
    ...
    SELECT
        ...
        A.SEGMENT_FLAG
        FROM ' || V_TABLEINSERT1 || ' A
        JOIN TMP_IFRS_EAD_PAYM_NONPRK_' || P_RUNID || ' B ON A.MASTERID = B.MASTERID
