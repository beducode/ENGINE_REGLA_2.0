...
BEGIN 
    ...
    SELECT
        ...
        SEGMENT_FLAG
    FROM ' || V_TABLEINSERT1 || ' A
    INNER JOIN IFRS_MASTER_PRODUCT_PARAM B ON A.DATA_SOURCE = B.DATA_SOURCE AND A.PRODUCT_TYPE_1 = B.PRD_TYPE_1
	WHERE A.DATA_SOURCE <> ''HASANAH_CARD'' AND A.IMPAIRED_FLAG = ''C''
    UNION ALL
    SELECT
        A.DOWNLOAD_DATE,
        A.MASTERID,
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE TMP_IFRS_EAD_RESULT_NONPRK_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE MINUS_LIFETIME AS
