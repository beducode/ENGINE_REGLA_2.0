...
BEGIN 
    ...
    FOR V_PD_RULE_ID IN
        EXECUTE 'SELECT DISTINCT PKID FROM IFRS_PD_RULES_CONFIG WHERE PD_METHOD = ''MAA'' AND ACTIVE_FLAG = 1 AND IS_DELETE  = 0'
    LOOP
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'SELECT (EXPECTED_LIFE/INCREMENT_PERIOD) INTO ' || V_MAX_SEQ || ' 
        FROM IFRS_PD_RULES_CONFIG 
        WHERE PD_METHOD = ''MAA'' AND ACTIVE_FLAG = 1 AND IS_DELETE = 0 AND PKID = ' || V_PD_RULE_ID || '';
        EXECUTE (V_STR_QUERY);
