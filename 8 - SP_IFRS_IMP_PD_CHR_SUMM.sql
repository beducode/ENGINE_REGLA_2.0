...
BEGIN 
    ...
    -------- ====== BODY ======
    DELETE FROM IFRS_PD_CHR_BASE_DATA A         
    USING IFRS_PD_RULES_CONFIG B WHERE A.PD_RULE_ID = B.PKID         
    AND DOWNLOAD_DATE = F_EOMONTH(V_CURRDATE, B.INCREMENT_PERIOD, 'M', 'PREV')
    ...
    INNER JOIN IFRS_PD_RULES_CONFIG B ON A.PD_RULE_ID = B.PKID WHERE PROJECTION_DATE = ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE        
    AND B.ACTIVE_FLAG = 1 AND B.IS_DELETE = 0
    ...
    FROM  CHR_PREV_' || P_RUNID || ' AS B, IFRS_PD_RULES_CONFIG AS C 
