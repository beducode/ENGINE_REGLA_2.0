...
BEGIN 
    ...
    ELSE 
        ...
        V_TABLEINSERT3 := 'IFRS_EAD_TERM_MONTHLY';
    ...
    IF P_PRC = 'S' THEN
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'DROP TABLE IF EXISTS IFRS_EAD_TERM_MONTHLY_' || P_RUNID || '';
        EXECUTE (V_STR_QUERY);

        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE IFRS_EAD_TERM_MONTHLY_' || P_RUNID || ' AS SELECT * FROM IFRS_EAD_TERM_MONTHLY WHERE 0=1';
        EXECUTE (V_STR_QUERY);
    END IF;
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DROP TABLE IF EXISTS TEMP_TABLE_' || P_RUNID || '';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE TEMP_TABLE_' || P_RUNID || ' 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS NCI_TEMP';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX NCI_TEMP ON TEMP_TABLE_' || P_RUNID || ' (CCF_RULES_ID ASC, EAD_MODEL_ID ASC, R_NUMBER ASC) ';
    EXECUTE (V_STR_QUERY);
