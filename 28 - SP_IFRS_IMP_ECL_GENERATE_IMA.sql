...
FROM
    ...
    LEFT JOIN IFRS_LGD_RULES_CONFIG G
    ON D.LGD_MODEL_ID = G.PKID
    LEFT JOIN IFRS_PD_RULES_CONFIG H
    ...
    LEFT JOIN IFRS_CCF_RULES_CONFIG L
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE UPDATE_IFRS_IMA_IMP_CURR_' || P_RUNID || ' AS
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TMP_QRY AS
    ...
    '' || COALESCE(CAST(A.CCF_FLAG AS VARCHAR(10)),' || '''''''''''''' || ') || '' AS CCF_FLAG, 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE UPDATE_TMP_IFRS_ECL_IMA AS
