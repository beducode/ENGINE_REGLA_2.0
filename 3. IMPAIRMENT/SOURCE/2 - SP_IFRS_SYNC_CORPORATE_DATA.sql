...
BEGIN 
    ...
    SELECT COUNT(*) AS COUNTS FROM ' || V_TABLEINSERT11 || ' WHERE DOWNLOAD_DATE = ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE
    UNION ALL 
    SELECT COUNT(*) AS COUNTS FROM ' || V_TABLEINSERT12 || ' WHERE DOWNLOAD_DATE = ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE
    ) A ' INTO V_CHECKROWS;
    ...
    IF (V_CHECKROWS > 0) THEN
            ...
            V_STR_QUERY := '';
            V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || V_TABLEINSERT9 || ' A      
            SET CUMMULATIVE_PD = (CAST(B.CUMMULATIVE_PD AS FLOAT) / 100)      
            ...
            FROM (      
            SELECT CAST(CAST(CUMMULATIVE_PD AS FLOAT) AS NUMERIC(32,6)) - CAST(COALESCE(LAG(CAST(CUMMULATIVE_PD AS FLOAT)) OVER (PARTITION BY DOWNLOAD_DATE, SEGMENT, RATING_CODE ORDER BY CAST(REMAINING_TENOR_YEAR AS INT)), 0) AS NUMERIC(32,6)) AS MARGINAL      
            , *      
            FROM ' || V_TABLEINSERT9 || '      
            WHERE DOWNLOAD_DATE = ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''     
            ) B 
