...
BEGIN 
    ...
    WHERE F_EOMONTH(DOWNLOAD_DATE, 0, ''M'', ''NEXT'') = ''' || CAST(V_CURRDATE AS VARCHAR(10)) || ''' ';
    EXECUTE (V_STR_QUERY);
    ...
    ,NOTIONAL_AMOUNT_ORIGINAL_CCY AS OUTSTANDING      
    ,START_DATE AS LOAN_START_DATE      
    ,MAT_DATE AS LOAN_DUE_DATE      
    ,INTEREST_RATE      
    ,INTEREST_RECEIVABLE_CCY AS INTEREST_ACCRUED      
    ...
    ,INTEREST_TYPE      
    ,CARRYING_AMOUNT AS FAIR_VALUE_AMOUNT      
    ,CASE PAYMENT_TERM       
    WHEN ''MONTHLY'' THEN ''M''      
    ...
    ,SOVEREIGN_FLAG      
    ,LAST_COUPON_DATE AS LAST_PAYMENT_DATE      
    ,NEXT_COUPON_DATE AS NEXT_PAYMENT_DATE      
    ,CONTRACT_ID AS ISIN_CODE      
    ,INV_TYPE      
    ,COLLECTABILITY AS BI_COLLECTABILITY      
    ,UNAMORTIZED_DISCOUNT_PREMIUM_IN_ORI_CCY AS UNAMORT_DISCOUNT_PREMIUM      
    ,DISCOUNT_PREMIUM_CCY AS DISCOUNT_PREMIUM_AMOUNT      
    ...
    ON A.CURRENCY = C.CURRENCY AND A.DOWNLOAD_DATE = C.DOWNLOAD_DATE            
    WHERE F_EOMONTH(DOWNLOAD_DATE, 0, ''M'', ''NEXT'') = ''' || CAST(V_CURRDATE AS VARCHAR(10)) || ''' ';
    EXECUTE (V_STR_QUERY);
