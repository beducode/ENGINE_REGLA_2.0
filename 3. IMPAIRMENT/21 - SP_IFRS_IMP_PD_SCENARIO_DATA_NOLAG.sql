...
BEGIN 
    ...
    LOOP
        ...
        V_STR_QUERY := V_STR_QUERY || 'INSERT INTO TMP_IFRS_PD_SCENARIO_DATA_NOLAG 
            ...
            JOIN ' || 'IFRS_PD_RULES_CONFIG' || ' B ON B.PKID = ' || V_SEGMENT.PD_RULE_ID || ' 
        ...
        V_STR_QUERY := V_STR_QUERY || ' AND ' || REPLACE(V_SEGMENT.CONDITION,'"','') || ' ';
        EXECUTE (V_STR_QUERY);
        
    END LOOP;

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'UPDATE TMP_IFRS_PD_SCENARIO_DATA_NOLAG A
        SET BUCKET_ID = E.BUCKET_ID, BUCKET_ID_ORIG = E.BUCKET_ID 
        FROM IFRS_BUCKET_HEADER C, IFRS_BUCKET_DETAIL E
        WHERE A.BUCKET_GROUP = C.BUCKET_GROUP AND C.IS_DELETE = 0  
        AND A.BUCKET_GROUP = E.BUCKET_GROUP AND E.IS_DELETE = 0 
        AND ((CASE WHEN C.OPTION_GROUPING = ''DPD'' THEN A.DAY_PAST_DUE 
    ...
    V_STR_QUERY := V_STR_QUERY || 'UPDATE TMP_IFRS_PD_SCENARIO_DATA_NOLAG A 
        ...
        WHERE A.DEFAULT_RULE_ID = B.RULE_ID 
        AND A.MASTERID = B.MASTERID 
        AND C.BUCKET_GROUP = A.BUCKET_GROUP';
    EXECUTE (V_STR_QUERY);
