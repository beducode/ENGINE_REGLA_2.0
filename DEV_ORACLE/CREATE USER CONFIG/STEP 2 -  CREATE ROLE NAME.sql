...
BEGIN
    ...
    FOR src IN (
        SELECT 'NTT_APPROVAL' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_AUDIT' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_CUSTOM_REPORT' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_DATA_MANAGENENT' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_EMAIL_NOTIFICATION' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_FILE_MANAGER' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_HANGFIRE' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_JOURNAL' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_PARAMETER' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_PLATFORM_SETTING' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_PSAK413_IMPAIRMENT' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_RISK_MODELLING' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_USER' AS schema_name FROM dual UNION ALL
        SELECT 'NTT_WORKFLOW' AS schema_name FROM dual
    )
    LOOP
