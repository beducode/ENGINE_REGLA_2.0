...
BEGIN 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE TOTAL_COUNT_BUCKET_' || P_RUNID || ' AS 
    SELECT PD_RULE_ID, BUCKET_ID, SUM(TOTAL_COUNT) AS TOTAL_COUNT_ALL 
    FROM IFRS_PD_CHR_SUMM A        
    INNER JOIN IFRS_PD_RULES_CONFIG B ON A.PD_RULE_ID = B.PKID AND B.IS_DELETE = 0 AND B.ACTIVE_FLAG= 1        
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE CHR_MASTER_' || P_RUNID || ' AS
    ...
    LOOP
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE BUCKET_CHR_' || P_RUNID || ' AS 
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE BUCKET_FULL_' || P_RUNID || ' AS 
        ...
        INNER JOIN IFRS_PD_RULES_CONFIG C ON A.BUCKET_GROUP = C.BUCKET_GROUP AND C.PKID = ' || V_PD_RULE_ID || '        
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE CUMULATIVE_' || P_RUNID || ' AS 
    SELECT A.* FROM ' || V_TABLEINSERT4 || ' A
    INNER JOIN IFRS_PD_RULES_CONFIG B  ON A.PD_RULE_ID = B.PKID              
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE TEMP TABLE CUMULATIVE_PD_RESULT_' || P_RUNID || ' AS 
