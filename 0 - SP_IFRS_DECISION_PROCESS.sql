...
BEGIN 
    /*
        ------ ### FOR QUERY SAMPLE ###
        ---- STEP 1
        IF P_STEP_DECISION = 1 THEN
            P_QUERY := 'SELECT * FROM IFRS_IMA_IMP_CURR_S_11111_1111 WHERE DOWNLOAD_DATE = ''20240531'' LIMIT 1';
        -- ---- STEP 2
        ELSEIF P_STEP_DECISION = 2 THEN
            P_QUERY := 'SELECT * FROM IFRS_IMA_IMP_CURR_S_11111_1111 WHERE DOWNLOAD_DATE = ''20240531'' AND DATA_SOURCE = ''LOAN'' LIMIT 2 OFFSET 5';
        -- ---- STEP 3
        ELSEIF P_STEP_DECISION = 3 THEN
            P_QUERY := 'SELECT * FROM IFRS_IMA_IMP_CURR_S_11111_1111 WHERE DOWNLOAD_DATE = ''20240531'' AND DATA_SOURCE = ''LOAN'' LIMIT 5 OFFSET 10';
        -- -- STEP 4
        ELSE
            P_QUERY := 'SELECT * FROM IFRS_IMA_IMP_CURR_S_11111_1111 WHERE DOWNLOAD_DATE = ''20240531'' AND DATA_SOURCE = ''LOAN'' LIMIT 10 OFFSET 15';
        END IF;
        -------- ### FOR QUERY SAMPLE ###
    */
    ...
    IF V_DECISION = 0 THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'INSERT INTO IFRS_DECISION_LOGS (DOWNLOAD_DATE, RUN_ID, PROCESS_ID, PROCESS_DATE)
        SELECT ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE, UPPER(''' || P_RUNID || '''), PG_BACKEND_PID(), CURRENT_DATE';
        EXECUTE (V_STR_QUERY);
    ...
    -- -------- ====== RESULT ======
    IF P_STEP_DECISION = P_STEP_MAX THEN
        V_QUERYS = 'SELECT * FROM ' ||  V_TABLENAME || ' WHERE EXISTS (' || P_QUERY || ')';
        RAISE NOTICE '---> %', V_QUERYS;
        CALL SP_IFRS_RESULT_PREV(V_CURRDATE, V_QUERYS, V_SP_NAME, V_RETURNROWS2, P_RUNID);
    ELSE
        V_QUERYS = 'SELECT * FROM ' ||  V_TABLENAMEDEC || ' WHERE EXISTS (' || P_QUERY || ')';
        RAISE NOTICE '---> %', V_QUERYS;
        CALL SP_IFRS_RESULT_PREV(V_CURRDATE, V_QUERYS, V_SP_NAME, V_RETURNROWS2, P_RUNID);
    END IF;
    -- -------- ====== RESULT ======
