---- DROP PROCEDURE SP_IFRS_IMP_DEFAULT_RULE_ECL_ECL;

CREATE OR REPLACE PROCEDURE SP_IFRS_IMP_DEFAULT_RULE_ECL_ECL(
...
BEGIN 
    ...
    LOOP
        ...
        INSERT INTO IFRS_SCENARIO_GENERATE_QUERY (
            ...
            V_TABLE_NAME,
            V_STR_SQL_RULE,
            'SP_IFRS_IMP_DEFAULT_RULE_ECL_ECL',
            CURRENT_DATE
        );
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'UPDATE ' || V_TABLEINSERT1 || ' X
    SET BUCKET_ID = CASE WHEN DEFAULT_FLAG = 0 THEN Y.BUCKET_ID ELSE X.BUCKET_ID END FROM (
    SELECT
        A.BUCKET_GROUP,
        MAX(BUCKET_ID) AS BUCKET_ID
    FROM
        IFRS_BUCKET_HEADER A
        JOIN IFRS_BUCKET_DETAIL B
        ON A.BUCKET_GROUP = B.BUCKET_GROUP
    GROUP BY
        A.BUCKET_GROUP
    ) Y
    WHERE X.BUCKET_GROUP = Y.BUCKET_GROUP AND X.DOWNLOAD_DATE = ''' || CAST(V_CURRDATE AS VARCHAR(10)) || ''' ';
    EXECUTE (V_STR_QUERY);

    RAISE NOTICE 'SP_IFRS_IMP_DEFAULT_RULE_ECL_ECL | AFFECTED RECORD : %', V_RETURNROWS2;
    -------- ====== BODY ======

    -------- ====== LOG ======
    V_TABLEDEST = V_TABLEINSERT2;
    V_COLUMNDEST = '-';
    V_SPNAME = 'SP_IFRS_IMP_DEFAULT_RULE_ECL_ECL';
    V_OPERATION = 'INSERT';
