---- DROP PROCEDURE SP_IFRS_IMP_PD_FL_YEAR_TO_MONTH_ECL;

CREATE OR REPLACE PROCEDURE SP_IFRS_IMP_PD_FL_YEAR_TO_MONTH_ECL(
...
BEGIN 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IDX_PD_FINAL_' || P_RUNID || ' ON PD_FINAL_' || P_RUNID || ' (DOWNLOAD_DATE, ECL_MODEL_ID, SEGMENTATION_ID, PD_RULE_ID, BUCKET_ID, FL_SEQ) ';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DROP INDEX IF EXISTS IDX_' || V_TABLEPDCONFIG || ' ';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IDX_' || V_TABLEPDCONFIG || ' ON ' || V_TABLEPDCONFIG || ' (PKID) WHERE ACTIVE_FLAG = 1 AND IS_DELETE = 0';
    EXECUTE (V_STR_QUERY);
    ...
    RAISE NOTICE 'SP_IFRS_IMP_PD_FL_YEAR_TO_MONTH_ECL | AFFECTED RECORD : %', V_RETURNROWS2;
    -------- ====== BODY ======
    ...
    V_COLUMNDEST = '-';
    V_SPNAME = 'SP_IFRS_IMP_PD_FL_YEAR_TO_MONTH_ECL';
    V_OPERATION = 'INSERT';
