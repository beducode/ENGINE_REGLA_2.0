DROP VIEW IF EXISTS VW_EAD_RULES_CONFIG CASCADE;
...
A.SEGMENTATION_ID,
CASE 
	WHEN A.INTEREST_ACCRUED = 1 AND CCF_FLAG = 0 THEN UPPER(EAD_BALANCE) || ''+'' || ''INTEREST_ACCRUED'' ----- "OUTSTANDING+INTEREST_ACCRUED"
	WHEN A.INTEREST_ACCRUED = 0 AND CCF_FLAG = 1 THEN ''('' || UPPER(EAD_BALANCE) || ''*CCF'' || '')'' ----- "(OUTSTANDING*CCF)"
	WHEN A.INTEREST_ACCRUED = 1 AND CCF_FLAG = 1 THEN ''(('' || UPPER(EAD_BALANCE) || ''+'' || ''INTEREST_ACCRUED'' || '')'' || ''*CCF'' || '')'' ----- ((OUTSTANDING-INTEREST_ACCRUED)*CCF)
...
MAX(CASE WHEN INCLUDE_ACCRUED_INTEREST = TRUE THEN 1 ELSE 0 END) AS INTEREST_ACCRUED,
...
WHERE B.IS_ACTIVE = TRUE')AS EAD_RULES_CONFIG(PKID BIGINT
