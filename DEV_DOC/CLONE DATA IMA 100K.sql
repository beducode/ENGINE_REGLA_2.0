...
$$
DECLARE
    duplicate_count INTEGER := 20000;
    max_pkid BIGINT;
BEGIN
    -- Ambil nilai maksimum PKID dari tabel target
    SELECT COALESCE(MAX(pkid), 0) INTO max_pkid FROM ifrs_master_account;

    -- Gunakan CTE untuk membangkitkan angka 1–20000
    WITH numbers AS (
        SELECT generate_series(1, duplicate_count) AS n
    )
    INSERT INTO IFRS_MASTER_ACCOUNT_100K (
        pkid,
        download_date,
        masterid,
        master_account_code,
